...
let
  ...
  # `hax-frontend-exporter` `cargo-hax` links too, even though `cargo-hax`'s own
  # code isn't written against the `rustc_private` sysroot crates it would
  # pull in (`gimli`, `std`, ...), so rustc reports those as duplicated.
  ...
  # default-member libraries and binaries. The selection below is
  # `default-members` of the workspace manifest minus `cargo-hax`; there is no
  # cargo flag expressing that, hence the enumeration.
  hax_driver_and_libs = craneLib.buildPackage (commonArgs // {
    inherit cargoArtifacts pname;
    doInstallCargoArtifacts = true;
    cargoExtraArgs = "--locked"
      + " -p hax-frontend-exporter -p hax-frontend-exporter-options"
      + " -p hax-driver -p test-driver -p hax-lib -p hax-lib-macros"
      + " -p hax-lib-macros-types -p hax-lib-protocol -p hax-lib-protocol-macros"
      + " -p hax-engine-names";
  });
