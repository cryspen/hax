...
def Tests.Legacy__side_effects__src__lib.Nested_return.other_fun
  (rng : i8)
  : Result
...
def Tests.Legacy__side_effects__src__lib.Nested_return.fun
  (rng : i8)
  : Result
  (Rust_primitives.Hax.Tuple2
    i8
  ...
  := do
  let ⟨tmp0, out⟩ ← (pure
    (← Tests.Legacy__side_effects__src__lib.Nested_return.other_fun rng));
...
structure Tests.Legacy__side_effects__src__lib.Issue_1299.Foo where
  y : u8

structure Tests.Legacy__side_effects__src__lib.Issue_1299.S where
  g : Tests.Legacy__side_effects__src__lib.Issue_1299.Foo

structure Tests.Legacy__side_effects__src__lib.Issue_1299.OtherS where
  g : (Core.Option.Option Tests.Legacy__side_effects__src__lib.Issue_1299.Foo)

def Tests.Legacy__side_effects__src__lib.Issue_1299.Impl.from
  (i : Tests.Legacy__side_effects__src__lib.Issue_1299.Foo)
  : Result Tests.Legacy__side_effects__src__lib.Issue_1299.Foo
  := do
  (Tests.Legacy__side_effects__src__lib.Issue_1299.Foo.mk
    (y := (← Core.Clone.Clone.clone
        (Tests.Legacy__side_effects__src__lib.Issue_1299.Foo.y i))))

structure Tests.Legacy__side_effects__src__lib.Issue_1299.Error where


def Tests.Legacy__side_effects__src__lib.Issue_1299.Impl_1.from
  (i : Tests.Legacy__side_effects__src__lib.Issue_1299.OtherS)
  : Result
  (Core.Result.Result
    Tests.Legacy__side_effects__src__lib.Issue_1299.S
    Tests.Legacy__side_effects__src__lib.Issue_1299.Error)
  := do
  (match
    (← Core.Option.Impl.ok_or
        Tests.Legacy__side_effects__src__lib.Issue_1299.Foo
        Tests.Legacy__side_effects__src__lib.Issue_1299.Error
        (← Core.Option.Impl.as_ref
            Tests.Legacy__side_effects__src__lib.Issue_1299.Foo
            (Tests.Legacy__side_effects__src__lib.Issue_1299.OtherS.g i))
        Tests.Legacy__side_effects__src__lib.Issue_1299.Error.mk)
  with
    | (Core.Result.Result.Ok hoist47)
      => do
        (Core.Result.Result.Ok
          (Tests.Legacy__side_effects__src__lib.Issue_1299.S.mk
            (g := (← Tests.Legacy__side_effects__src__lib.Issue_1299.Impl.from
                hoist47))))
    | (Core.Result.Result.Err err) => do (Core.Result.Result.Err err))

class Tests.Legacy__side_effects__src__lib.Issue_1083.MyFrom
  (Self : Type) (T : Type)
  where
  my_from : T -> Result Self

instance Tests.Legacy__side_effects__src__lib.Issue_1083.Impl :
  Tests.Legacy__side_effects__src__lib.Issue_1083.MyFrom u16 u8
  where
  my_from (x : u8) := do (← Rust_primitives.Hax.cast_op x)

def Tests.Legacy__side_effects__src__lib.Issue_1083.f
  (x : u8)
  ...
  with
    ...
    | (Core.Ops.Control_flow.ControlFlow.Continue val)
      => do
        let _ ← (pure val);
        (Core.Result.Result.Ok
          (← Tests.Legacy__side_effects__src__lib.Issue_1083.MyFrom.my_from x)))

def Tests.Legacy__side_effects__src__lib.add3
  (x : u32)
  (y : u32)
  (z : u32)
  : Result u32
...
def Tests.Legacy__side_effects__src__lib.local_mutation
  (x : u32)
  : Result u32
  := do
        ...
        | TODO_LINE_622
          => do
            ...
            (Rust_primitives.Hax.Tuple2.mk
              ...
              (← Tests.Legacy__side_effects__src__lib.add3
                  x
...
def Tests.Legacy__side_effects__src__lib.early_returns
  (x : u32)
  : Result u32
  := do
  (← if (← Rust_primitives.Hax.Machine_int.gt x (3 : u32)) then do
...
def Tests.Legacy__side_effects__src__lib.simplifiable_return
  (c1 : Bool)
  (c2 : Bool)
  (c3 : Bool)
  : Result i32
...
def Tests.Legacy__side_effects__src__lib.simplifiable_question_mark
  (c : Bool)
...
def Tests.Legacy__side_effects__src__lib.direct_result_question_mark
...
def Tests.Legacy__side_effects__src__lib.direct_result_question_mark_coercion
...
def Tests.Legacy__side_effects__src__lib.options
...
def Tests.Legacy__side_effects__src__lib.question_mark
  (x : u32)
...
structure Tests.Legacy__side_effects__src__lib.A where


structure Tests.Legacy__side_effects__src__lib.B where


def Tests.Legacy__side_effects__src__lib.monad_lifting
  (x : u8)
  : Result
  (Core.Result.Result
    Tests.Legacy__side_effects__src__lib.A
    Tests.Legacy__side_effects__src__lib.B)
  := do
  (← if (← Rust_primitives.Hax.Machine_int.gt x (123 : u8)) then do
    (match
      (Core.Result.Result.Err Tests.Legacy__side_effects__src__lib.B.mk)
    with
      | (Core.Result.Result.Ok hoist35) => do (Core.Result.Result.Ok hoist35)
      | (Core.Result.Result.Err err) => do (Core.Result.Result.Err err))
  else do
    (Core.Result.Result.Ok Tests.Legacy__side_effects__src__lib.A.mk))

structure Tests.Legacy__side_effects__src__lib.Bar where
  a : Bool
  b : (Rust_primitives.Hax.Tuple2
      (RustArray (Rust_primitives.Hax.Tuple2 Bool Bool) (6 : usize))
      Bool)

structure Tests.Legacy__side_effects__src__lib.Foo where
  x : Bool
  y : (Rust_primitives.Hax.Tuple2
      Bool
      (Alloc.Vec.Vec
        Tests.Legacy__side_effects__src__lib.Bar
        Alloc.Alloc.Global))
  z : (RustArray Tests.Legacy__side_effects__src__lib.Bar (6 : usize))
  bar : Tests.Legacy__side_effects__src__lib.Bar

def Tests.Legacy__side_effects__src__lib.assign_non_trivial_lhs
  (foo : Tests.Legacy__side_effects__src__lib.Foo)
  : Result Tests.Legacy__side_effects__src__lib.Foo
  := do
  let foo : Tests.Legacy__side_effects__src__lib.Foo ← (pure
...
def Tests.Legacy__side_effects__src__lib.Issue_1300.fun
...
def Tests.Legacy__side_effects__src__lib.Issue_1089.test
  (x : (Core.Option.Option i32))
