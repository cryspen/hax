...
def Tests.Legacy__guards__src__lib.if_let_guard
...
def Tests.Legacy__guards__src__lib.equivalent
  (x : (Core.Option.Option (Core.Result.Result i32 i32)))
  : Result i32
  := do
  (match x with
...
def Tests.Legacy__guards__src__lib.multiple_guards
  (x : (Core.Option.Option (Core.Result.Result i32 i32)))
  : Result i32
  := do
  (match x with
...
def Tests.Legacy__guards__src__lib.if_guard
