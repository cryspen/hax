...
//! follows, from highest to lowest precedence: the member crate's
//! `hax.toml`, the workspace-root `hax.toml`, and the built-in defaults
...
/// resolution (a `path` entry in `hax.toml`) points at the executable whose
/// name matches the tool; the remaining executables are resolved by name in
/// the same directory, and a missing sibling is an error. Whenever the
/// resolution deviates from the built-in default, a one-line notice names
/// the version this release was tested with.
pub fn provide_tool(
    tool: &str,
...
/// deviation from the built-in default. Nothing is installed: these
/// versions are known to hax but provisioned by the user, so this is the
/// resolution plus the same notice [`provide_tool`] prints.
pub fn provide_version(
    key: &str,
    member: Option<&config::HaxToml>,
    workspace: Option<&config::HaxToml>,
    message_format: MessageFormat,
    ...
    let defaults = defaults::defaults();
    let default_version = &defaults.versions[key];
    let version = match resolve::resolve_version(key, member, workspace, defaults).kind {
        resolve::Resolved::Version(version) => version,
        resolve::Resolved::Path(_) => {
            unreachable!("`[versions]` entries always resolve to versions")
        }
    };
    if version != *default_version {
        HaxMessage::NonDefaultToolVersion {
