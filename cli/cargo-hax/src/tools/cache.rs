...
/// layout is used on every platform.
pub fn cache_root() -> Result<PathBuf, String> {
    let var = |name| std::env::var(name).ok();
    cache_root_of(var("XDG_CACHE_HOME").as_deref(), var("HOME").as_deref())
}

/// [`cache_root`] over explicit environment values, so the layout can be
/// tested on every platform without mutating the process environment. An
/// empty or relative value counts as unset: the XDG specification requires
/// it, and a path relative to the invocation directory would not resolve to
/// where the install went, tool executables being spawned with the crate
/// root as their working directory.
fn cache_root_of(xdg_cache_home: Option<&str>, home: Option<&str>) -> Result<PathBuf, String> {
    ...
    if let Some(dir) = absolute(xdg_cache_home) {
        return Ok(dir.join("hax"));
    }
    match absolute(home) {
        Some(home) => Ok(home.join(".cache").join("hax")),
        None => Err(
            "cannot locate the hax cache: neither XDG_CACHE_HOME nor HOME is set to an \
             absolute path"
                .into(),
        ),
    }
...
pub fn clean() -> Result<usize, String> {
    let root = cache_root()?;
    ...
    staging
        .close()
        .map_err(|e| format!("could not delete the cache: {e}"))?;
    Ok(removed)
}
...
mod tests {
    ...
    /// The cache layout is the XDG one on every platform, including macOS
    /// where `~/Library/Caches` would be the platform convention. The
    /// documented paths are part of the interface: users delete entries and
    /// CI caches this directory by path.
    #[test]
    fn cache_root_is_xdg_on_every_platform() {
        assert_eq!(
            cache_root_of(Some("/xdg"), Some("/home/user")).unwrap(),
            Path::new("/xdg/hax")
        );
        // No XDG_CACHE_HOME, an empty one, or a relative one (which the
        // XDG specification says to ignore): `~/.cache/hax`, never a
        // platform-specific location.
        for xdg in [None, Some(""), Some(".cache"), Some("../cache")] {
            assert_eq!(
                cache_root_of(xdg, Some("/home/user")).unwrap(),
                Path::new("/home/user/.cache/hax")
            );
        }
        // Neither usable: an error rather than a path resolved against the
        // current directory, which tool executables do not run in.
