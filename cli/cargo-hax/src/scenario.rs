...
fn extract(
    ...
    // directory no selected scenario owns.
    if full_scope {
        warn_orphan_directories(&project, &scope, message_format);
    }
...
/// resolved output directory in scope accounts for and that are not a
/// backend directory of the scenario-less layout: typically the leftovers
/// of a renamed scenario, including `Verification/` content.
fn warn_orphan_directories(
    project: &ProjectContext,
    scope: &[ResolvedScenario],
    message_format: MessageFormat,
) {
    let members: Vec<_> = if project.workspace_scope() {
        project.members.iter().collect()
    } else {
        // A member-scope run sees all scenarios of that member, so its
        // `proofs/` directory can still be checked soundly.
        let member = &project
            .root_package
            .as_ref()
...
mod tests {
    ...
    /// `output-dir` allows a `..` component, so a scenario can name a
    /// directory outside its own member; one that resolves to a crate root
    /// would scaffold the generated Lean package over that crate's sources.
    #[test]
    fn an_output_directory_on_a_package_root_is_rejected() {
        ...
        // A directory inside another member is not that member's root: a
        // shared extraction tree is the reason `..` is allowed.
