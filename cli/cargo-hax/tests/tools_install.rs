...
fn platform_without_artifact_names_the_escape_hatch() {
    ...
    let (output, success) = env.run(&["tools", "install", "charon@v1"]);
    assert!(!success);
    assert!(output.contains("`path` entry in hax.toml"), "{output}");
    // The platforms that do have artifacts, so someone on a platform hax
    // does not publish for learns that from the error itself.
