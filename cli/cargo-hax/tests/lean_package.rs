...
/// A stub aeneas that emulates the extraction: it writes `Types.lean` and
/// `Funs.lean` under `<-dest>/<-subdir>/`, keeping the last occurrence of
/// each flag like the real tool. With `externals`, it additionally
/// generates a template for the models of external definitions.
...
fn first_run_scaffolds_a_complete_package() {
    ...
    // The root module imports exactly the files this run produced, plus the
    // Verification stub, in build order.
    assert_eq!(
...
fn an_unimported_file_and_a_stale_import_are_warned_about() {
    ...
    // A pre-existing sparse root module: it misses `Types` and the
    // Verification stub, and imports a `Specs` the extraction does not
    // produce. The commented import opts out warning-free.
    std::fs::write(
    ...
    assert!(success, "{output}");
    assert!(
        output.contains("does not import MyFixture.Extraction.Types"),
        "{output}"
    );
    assert!(
...
fn external_models_are_seeded_into_assumptions_and_reached_through_a_shim() {
    ...
    let (output, success) = run_backend(&crate_dir, &[]);
    assert!(success, "{output}");
    // Seeded from the aeneas template; the shim points the generated
    // imports at it.
    assert_eq!(read(&assumptions), "-- fill in the external functions\n");
    assert_eq!(read(&shim), "import MyFixture.Assumptions.FunsExternal\n");
    // Neither the shim nor the template is expected in the root module.
    assert!(!output.contains("does not import"), "{output}");
...
fn project_files_false_disables_scaffolding_but_not_the_clearing() {
    ...
    // Extraction/ stays hax-owned: stale files are still cleared.
    assert!(!stale.exists());
    assert!(lean_dir.join("MyFixture/Extraction/Funs.lean").is_file());
}
