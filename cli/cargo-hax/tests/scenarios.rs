...
fn dry_run_prints_the_resolved_invocations_and_runs_nothing() {
    ...
    // the printed output-dir is where the files actually land. The printed
    // path derives from `cargo metadata`, which resolves symlinks, so the
    // expectation must resolve them too (macOS puts temp directories behind
    ...
    assert!(
        output.contains(&format!(
            ...
            crate_dir
                .canonicalize()
                .unwrap()
                .join("proofs/book/fstar/extraction")
                .display()
        )),
        "{output}"
    );
