...
use common::{cargo_hax, command, output_of, path_entries, stub, stub_pipeline_tools, write_crate};
...
fn project() -> tempfile::TempDir {
    ...
    stub_pipeline_tools(&stubs, &stub("aeneas-invoked"));
    std::fs::write(root.join("hax.toml"), path_entries(&stubs)).unwrap();
    dir
}
