...
// Source map parsing
// ─────────────────────────────────────────────────────────────────────────────
function decodeSourceMap(mapFilePath) {
    let raw;
    try {
        raw = JSON.parse(fs.readFileSync(mapFilePath, 'utf8'));
    }
    catch {
        return null;
    }
    const sources = raw.sources ?? [];
    const sourcesContent = raw.sourcesContent ?? sources.map(() => null);
    const encoded = raw.mappings ?? '';
    const mappings = [];
    let genLine = 0;
...
function resolveSources(sources, mapFilePath) {
    ...
    for (let idx = 0; idx < sources.length; idx++) {
        ...
        for (const folder of vscode.workspace.workspaceFolders ?? []) {
            if (tryAdd(path.join(folder.uri.fsPath, src))) {
                found = true;
                break;
            }
        }
...
class HaxManager {
    ...
    // ── Source map access ────────────────────────────────────────────────────
    loadSourceMap(leanPath) {
        if (this.smCache.has(leanPath)) {
            return this.smCache.get(leanPath);
        }
        ...
        if (sm) {
            ...
            for (const [, resolved] of sm.resolvedPaths) {
                if (!this.rustToLean.has(resolved)) {
                    this.rustToLean.set(resolved, new Set());
                }
                this.rustToLean.get(resolved).add(leanPath);
            }
            // Update context key for rust files
            vscode.commands.executeCommand('setContext', 'hax.hasReverseMap', this.rustToLean.size > 0);
        }
        return sm;
    }
    getColorMap(leanPath) {
        if (this.colorMapCache.has(leanPath)) {
            return this.colorMapCache.get(leanPath);
        }
    ...
    update(editor) {
        ...
        if (p.endsWith('.lean')) {
            this.updateLean(editor);
        }
        else if (p.endsWith('.rs')) {
            this.updateRust(editor);
        }
    }
    // ── Lean file update ─────────────────────────────────────────────────────
    updateLean(editor) {
        ...
        if (this.rainbowEnabled) {
            ...
            for (let i = 0; i < N; i++) {
                editor.setDecorations(this.decTypes.bg[i], buckets[i]);
            }
        }
        else {
            for (let i = 0; i < N; i++) {
        ...
        if (activeMappings.length > 0) {
            ...
            editor.setDecorations(this.decTypes.active[ci], activeMappings.map(m => new vscode.Range(m.genLine, 0, m.genLine, 0)));
    ...
    updateRust(editor) {
        ...
        for (const leanPath of leanPaths) {
            ...
            const rustLineMappings = sm.mappings.filter(m => m.srcIdx === srcIdx && m.srcLine === cursor.line);
            if (rustLineMappings.length > 0) {
                const ci = colorMap.get(rustLineMappings[0].genLine) ?? 0;
                for (let i = 0; i < N; i++) {
                    editor.setDecorations(this.decTypes.active[i], []);
                }
                editor.setDecorations(this.decTypes.active[ci], [new vscode.Range(cursor.line, 0, cursor.line, 0)]);
            }
            // Highlight lean editor if visible
            for (const ve of vscode.window.visibleTextEditors) {
                if (ve.document.uri.fsPath !== leanPath) {
                    continue;
                }
                const leanMappings = sm.mappings.filter(m => m.srcIdx === srcIdx && m.srcLine === cursor.line);
                for (let i = 0; i < N; i++) {
                    ve.setDecorations(this.decTypes.active[i], []);
                }
                if (leanMappings.length > 0) {
                    const ci = colorMap.get(leanMappings[0].genLine) ?? 0;
                    ve.setDecorations(this.decTypes.active[ci], leanMappings.map(m => new vscode.Range(m.genLine, 0, m.genLine, 0)));
                }
            }
        }
    }
    applyRainbowToRust(editor, sm, colorMap, _leanPath, activeMapping, activeCi) {
        const rustPath = normPath(editor.document.uri.fsPath);
        const srcIdx = [...sm.resolvedPaths.entries()].find(([, p]) => p === rustPath)?.[0];
        if (srcIdx === undefined) {
            return;
        ...
        if (this.rainbowEnabled) {
            ...
            const rustLineColor = new Map();
            for (const m of sm.mappings) {
                if (m.srcIdx !== srcIdx) {
                    continue;
                }
                if (rustLineColor.has(m.srcLine)) {
                    continue;
                }
                const ci = colorMap.get(m.genLine);
                if (ci !== undefined) {
                    rustLineColor.set(m.srcLine, ci);
                }
            }
        ...
        if (activeMapping !== null && activeMapping.srcIdx === srcIdx) {
            ...
            editor.setDecorations(this.decTypes.active[activeCi], [new vscode.Range(activeMapping.srcLine, 0, activeMapping.srcLine, 0)]);
        }
    }
    clearAll(editor) {
        for (let i = 0; i < N; i++) {
    ...
    toggleRainbow() {
        ...
        vscode.window.showInformationMessage(`Hax rainbow colours ${this.rainbowEnabled ? 'enabled' : 'disabled'}`);
        for (const editor of vscode.window.visibleTextEditors) {
            this.update(editor);
        }
    }
...
function buildColorMap(sm) {
    ...
    for (const [genLine, m] of [...lineRep.entries()].sort((a, b) => a[0] - b[0])) {
        ...
        if (!keyToColor.has(key)) {
            keyToColor.set(key, next++ % N);
        }
...
function closestMapping(mappings, line, col) {
    let best = null;
    for (const m of mappings) {
        if (m.genLine > line) {
            break;
        }
        if (m.genLine < line || m.genCol <= col) {
            best = m;
        }
    }
    return best;
 ...
 */
function findActiveSegment(sm, colorMap, cursorLine, cursorCol) {
    ...
    return sm.mappings.filter(m => {
        if (m.srcIdx !== rep.srcIdx || m.srcLine !== rep.srcLine) {
            return false;
        }
        if (seen.has(m.genLine)) {
            return false;
        }
        seen.add(m.genLine);
        return true;
...
function activate(ctx) {
    ...
    ctx.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(e => { if (e) {
        mgr.update(e);
    } }), vscode.window.onDidChangeTextEditorSelection(e => { mgr.update(e.textEditor); }), vscode.window.onDidChangeVisibleTextEditors(editors => {
    ...
    }), vscode.commands.registerCommand('hax.jumpToSource', () => mgr.jumpToSource()), vscode.commands.registerCommand('hax.jumpToLean', () => mgr.jumpToLean()), vscode.commands.registerCommand('hax.toggleRainbow', () => mgr.toggleRainbow()), vscode.languages.registerHoverProvider([{ language: 'lean4' }, { language: 'lean' }, { pattern: '**/*.lean' }], new HaxHoverProvider(mgr)));
}
function deactivate() { }
