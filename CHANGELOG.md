...
Changes to hax-lib:
 ...
 - Add `#[hax_lib::symbolic_model("op")]` and `#[hax_lib::symbolic_trace]`: backend-neutral annotations mapping an abstract function onto a generic symbolic-library op, so one annotation drives both the extracted ProVerif model and a runtime symbolic trace (#2072)
