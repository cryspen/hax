...
class Tests.Legacy__dyn__src__lib.Printable (Self : Type) (S : Type) where
  stringify : Self -> Result S

instance Tests.Legacy__dyn__src__lib.Impl :
  Tests.Legacy__dyn__src__lib.Printable i32 Alloc.String.String
  where
  stringify (self : i32) := do (← Alloc.String.ToString.to_string self)

def Tests.Legacy__dyn__src__lib.print
 ...
 Alloc.Alloc.Global))
  ...
  let args : (RustArray Core.Fmt.Rt.Argument (1 : usize)) ← (pure
    #v[(← Core.Fmt.Rt.Impl.new_display Alloc.String.String
             (← Tests.Legacy__dyn__src__lib.Printable.stringify a))]);
  let _ ← (pure
    (← Std.Io.Stdio._print
