module New_tests.Rustc_coverage__overflow
...
let might_overflow (to_add: u32) : u32 =
    ...
    then
      let _:Prims.unit =
        Std.Io.Stdio.e_print (Core_models.Fmt.Rt.impl_1__new_const (mk_usize 1)
              (let unfold list = ["this will probably overflow\n"] in
  ...
  let _:Prims.unit =
    Std.Io.Stdio.e_print (Core_models.Fmt.Rt.impl_1__new_const (mk_usize 1)
          (let unfold list = ["continuing after overflow check\n"] in
            FStar.Pervasives.assert_norm (Prims.eq2 (List.Tot.length list) 1);
            Rust_primitives.Hax.array_of_list 1 list)
        <:
        Core_models.Fmt.t_Arguments)
          ...
          let _:Prims.unit =
            ...
            then
              ...
              let args:t_Array Core_models.Fmt.Rt.t_Argument (mk_usize 1) =
                let unfold list = [Core_models.Fmt.Rt.impl__new_display #u32 args] in
                FStar.Pervasives.assert_norm (Prims.eq2 (List.Tot.length list) 1);
                Rust_primitives.Hax.array_of_list 1 list
              in
              let _:Prims.unit =
