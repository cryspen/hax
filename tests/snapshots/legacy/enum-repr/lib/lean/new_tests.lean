...
def EnumWithRepr_cast_to_repr (x : EnumWithRepr) : RustM u16 := do
  match x with
    | (EnumWithRepr.ExplicitDiscr1 ) =>
      (pure EnumWithRepr.ExplicitDiscr1.AnonConst)
    | (EnumWithRepr.ExplicitDiscr2 ) =>
      (pure EnumWithRepr.ExplicitDiscr2.AnonConst)
    | (EnumWithRepr.ImplicitDiscrEmptyTuple ) =>
      (EnumWithRepr.ExplicitDiscr2.AnonConst +? (1 : u16))
    | (EnumWithRepr.ImplicitDiscrEmptyStruct ) =>
...
def ImplicitReprs_cast_to_repr (x : ImplicitReprs) : RustM u64 := do
  match x with
    | (ImplicitReprs.A ) => (pure (0 : u64))
    | (ImplicitReprs.B ) => (pure (1 : u64))
    | (ImplicitReprs.C ) => (pure (2 : u64))
    | (ImplicitReprs.D ) => (pure (3 : u64))
    | (ImplicitReprs.E ) => (pure ImplicitReprs.E.AnonConst)
    | (ImplicitReprs.F ) => (ImplicitReprs.E.AnonConst +? (1 : u64))
    | (ImplicitReprs.G ) => (ImplicitReprs.E.AnonConst +? (2 : u64))
    | (ImplicitReprs.H ) => (ImplicitReprs.E.AnonConst +? (3 : u64))
    | (ImplicitReprs.I ) => (ImplicitReprs.E.AnonConst +? (4 : u64))
