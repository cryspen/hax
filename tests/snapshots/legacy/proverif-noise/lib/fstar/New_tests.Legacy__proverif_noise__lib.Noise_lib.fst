module New_tests.Legacy__proverif_noise__lib.Noise_lib
...
let encrypt_with_ad (cs: t_CipherState) (ad plaintext: t_Slice u8)
  ...
  then
    ...
    (New_tests.Legacy__proverif_noise__lib.Noise_crypto.Error_CryptoError
      <:
      New_tests.Legacy__proverif_noise__lib.Noise_crypto.t_Error)
    <:
    Core_models.Result.t_Result (t_CipherState & Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
    ...
    | Core_models.Option.Option_Some k ->
      let cip:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
        New_tests.Legacy__proverif_noise__lib.Noise_crypto.encrypt (Alloc.Vec.impl_1__as_slice k
            <:
            t_Slice u8)
          n
          ad
...
let decrypt_with_ad (cs: t_CipherState) (ad ciphertext: t_Slice u8)
  ...
  then
    ...
    | Core_models.Option.Option_Some k ->
      (match
          New_tests.Legacy__proverif_noise__lib.Noise_crypto.decrypt (Alloc.Vec.impl_1__as_slice k
              <:
              t_Slice u8)
            n
            ad
...
let rekey (cs: t_CipherState)
  ...
  | Core_models.Option.Option_Some k ->
    let new_k:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
      New_tests.Legacy__proverif_noise__lib.Noise_crypto.rekey (Alloc.Vec.impl_1__as_slice k
          <:
          t_Slice u8)
    in
    Core_models.Result.Result_Ok
...
let initialize_symmetric (protocol_name: t_Slice u8) : t_SymmetricState =
  ...
  let (hv: Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global):Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
    if pnlen <. New_tests.Legacy__proverif_noise__lib.Noise_crypto.v_HASHLEN
    then
      Alloc.Slice.impl__concat #(t_Slice u8)
        ...
        ((let list =
          ...
          <:
          t_Slice (t_Slice u8))
    else New_tests.Legacy__proverif_noise__lib.Noise_crypto.hash protocol_name
  in
...
let mix_key (st: t_SymmetricState) (input_key_material: t_Slice u8) : t_SymmetricState =
  ...
  (ck: Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global), (temp_k: Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global) =
    New_tests.Legacy__proverif_noise__lib.Noise_crypto.hkdf2 (Alloc.Vec.impl_1__as_slice ck
        <:
        t_Slice u8)
      input_key_material
  in
  let temp_k:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
    if New_tests.Legacy__proverif_noise__lib.Noise_crypto.v_HASHLEN =. mk_usize 64
    then
      let temp_k:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
...
let mix_key_and_hash (st: t_SymmetricState) (input_key_material: t_Slice u8) : t_SymmetricState =
  ...
  (temp_k: Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global) =
    New_tests.Legacy__proverif_noise__lib.Noise_crypto.hkdf3 (Alloc.Vec.impl_1__as_slice ck
        <:
        t_Slice u8)
      input_key_material
  in
  ...
  let new_h:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
    Alloc.Vec.impl_2__extend_from_slice #u8
      ...
      new_h
      (Alloc.Vec.impl_1__as_slice temp_h <: t_Slice u8)
  in
  let new_h:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
    New_tests.Legacy__proverif_noise__lib.Noise_crypto.hash (Alloc.Vec.impl_1__as_slice new_h
        <:
        t_Slice u8)
  in
  let temp_k:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
...
let encrypt_and_hash (st: t_SymmetricState) (plaintext: t_Slice u8)
    : Core_models.Result.t_Result (t_SymmetricState & Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
      New_tests.Legacy__proverif_noise__lib.Noise_crypto.t_Error =
  match
    encrypt_with_ad st.f_cs (Alloc.Vec.impl_1__as_slice st.f_h <: t_Slice u8) plaintext
    <:
    Core_models.Result.t_Result (t_CipherState & Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
      New_tests.Legacy__proverif_noise__lib.Noise_crypto.t_Error
  with
...
let decrypt_and_hash (st: t_SymmetricState) (ciphertext: t_Slice u8)
    : Core_models.Result.t_Result (t_SymmetricState & Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
      New_tests.Legacy__proverif_noise__lib.Noise_crypto.t_Error =
  match
    decrypt_with_ad st.f_cs (Alloc.Vec.impl_1__as_slice st.f_h <: t_Slice u8) ciphertext
    <:
    Core_models.Result.t_Result (t_CipherState & Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
      New_tests.Legacy__proverif_noise__lib.Noise_crypto.t_Error
  with
...
let split (st: t_SymmetricState)
  ...
  (temp_k2: Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global) =
    New_tests.Legacy__proverif_noise__lib.Noise_crypto.hkdf2 (Alloc.Vec.impl_1__as_slice st.f_ck
        <:
        t_Slice u8)
      (Alloc.Vec.impl_1__as_slice (Alloc.Vec.impl__new #u8 ()
            <:
            Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
        <:
        t_Slice u8)
