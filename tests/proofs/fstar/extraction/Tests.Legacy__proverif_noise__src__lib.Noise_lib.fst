module Tests.Legacy__proverif_noise__src__lib.Noise_lib
...
let encrypt_with_ad (cs: t_CipherState) (ad plaintext: t_Slice u8)
  ...
  then
    ...
    (Tests.Legacy__proverif_noise__src__lib.Noise_crypto.Error_CryptoError
      <:
      Tests.Legacy__proverif_noise__src__lib.Noise_crypto.t_Error)
    <:
    Core.Result.t_Result (t_CipherState & Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
    ...
    | Core.Option.Option_Some k ->
      let cip:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
        Tests.Legacy__proverif_noise__src__lib.Noise_crypto.encrypt (Core.Ops.Deref.f_deref #(Alloc.Vec.t_Vec
...
let decrypt_with_ad (cs: t_CipherState) (ad ciphertext: t_Slice u8)
  ...
  then
    ...
    | Core.Option.Option_Some k ->
      (match
          Tests.Legacy__proverif_noise__src__lib.Noise_crypto.decrypt (Core.Ops.Deref.f_deref #(Alloc.Vec.t_Vec
                    u8 Alloc.Alloc.t_Global)
                #FStar.Tactics.Typeclasses.solve
                k
              <:
...
let rekey (cs: t_CipherState)
    : Core.Result.t_Result t_CipherState Tests.Legacy__proverif_noise__src__lib.Noise_crypto.t_Error =
  let { f_k = k ; f_n = n }:t_CipherState = cs in
  match k <: Core.Option.t_Option (Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global) with
  | Core.Option.Option_Some k ->
    let new_k:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
      Tests.Legacy__proverif_noise__src__lib.Noise_crypto.rekey (Core.Ops.Deref.f_deref #(Alloc.Vec.t_Vec
                u8 Alloc.Alloc.t_Global)
            #FStar.Tactics.Typeclasses.solve
            k
          <:
...
let initialize_symmetric (protocol_name: t_Slice u8) : t_SymmetricState =
  ...
  let (hv: Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global):Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
    if pnlen <. Tests.Legacy__proverif_noise__src__lib.Noise_crypto.v_HASHLEN
    then
      Alloc.Slice.impl__concat #(t_Slice u8)
        ...
        ((let list =
          ...
          <:
          t_Slice (t_Slice u8))
    else Tests.Legacy__proverif_noise__src__lib.Noise_crypto.hash protocol_name
  in
...
let mix_key (st: t_SymmetricState) (input_key_material: t_Slice u8) : t_SymmetricState =
  ...
  =
    Tests.Legacy__proverif_noise__src__lib.Noise_crypto.hkdf2 (Core.Ops.Deref.f_deref #(Alloc.Vec.t_Vec
              u8 Alloc.Alloc.t_Global)
          #FStar.Tactics.Typeclasses.solve
          ck
        <:
  ...
  let temp_k:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
    if Tests.Legacy__proverif_noise__src__lib.Noise_crypto.v_HASHLEN =. mk_usize 64
    then
      let temp_k:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
...
let mix_hash (st: t_SymmetricState) (data: t_Slice u8) : t_SymmetricState =
  let { f_cs = cs ; f_ck = ck ; f_h = h }:t_SymmetricState = st in
  {
    ...
    f_h
    =
    Tests.Legacy__proverif_noise__src__lib.Noise_crypto.hash (Core.Ops.Deref.f_deref #(Alloc.Vec.t_Vec
              u8 Alloc.Alloc.t_Global)
          #FStar.Tactics.Typeclasses.solve
          (Alloc.Slice.impl__concat #(t_Slice u8)
              #u8
...
let mix_key_and_hash (st: t_SymmetricState) (input_key_material: t_Slice u8) : t_SymmetricState =
  ...
  let ck, temp_h, temp_k:(Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global &
    Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global &
    Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global) =
    Tests.Legacy__proverif_noise__src__lib.Noise_crypto.hkdf3 (Core.Ops.Deref.f_deref #(Alloc.Vec.t_Vec
              u8 Alloc.Alloc.t_Global)
          #FStar.Tactics.Typeclasses.solve
          ck
        <:
