module Tests.Legacy__side_effects__src__lib
...
let early_returns (x: u32) : u32 =
  ...
  then mk_u32 0
    ...
    then
      ...
      | true -> mk_u32 34
      | _ ->
        let x, hoist31:(u32 & u32) = x, mk_u32 3 <: (u32 & u32) in
        Core.Num.impl_u32__wrapping_add (Core.Num.impl_u32__wrapping_add (mk_u32 123) hoist31 <: u32
          )
          x
    else
      let x:u32 = x +! mk_u32 9 in
      let x, hoist31:(u32 & u32) = x, x +! mk_u32 1 <: (u32 & u32) in
      Core.Num.impl_u32__wrapping_add (Core.Num.impl_u32__wrapping_add (mk_u32 123) hoist31 <: u32)
        x
...
let simplifiable_question_mark (c: bool) (x: Core.Option.t_Option i32) : Core.Option.t_Option i32 =
  ...
  then
    ...
    | Core.Option.Option_Some hoist36 ->
      let a:i32 = hoist36 +! mk_i32 10 in
...
let direct_result_question_mark_coercion (y: Core.Result.t_Result i8 u16)
  ...
  | Core.Result.Result_Ok hoist37 -> Core.Result.Result_Ok hoist37 <: Core.Result.t_Result i8 u32
  ...
  | Core.Option.Option_Some hoist41 ->
    ...
    then
      ...
      | Core.Option.Option_Some hoist43 ->
        (match
            Core.Option.Option_Some (Core.Num.impl_u8__wrapping_add hoist43 (mk_u8 3))
            <:
            Core.Option.t_Option u8
          with
          | Core.Option.Option_Some hoist49 ->
            (match hoist49 <: u8 with
              | Rust_primitives.Integers.MkInt 3 ->
                (match Core.Option.Option_None <: Core.Option.t_Option u8 with
                  | Core.Option.Option_Some some ->
                    let v:u8 = some in
                    (match x <: Core.Option.t_Option u8 with
                      | Core.Option.Option_Some hoist50 ->
                        (match y <: Core.Option.t_Option u8 with
                          | Core.Option.Option_Some hoist51 ->
              ...
              | Rust_primitives.Integers.MkInt 4 ->
                (match z <: Core.Option.t_Option u64 with
                  | Core.Option.Option_Some hoist38 ->
                    let v:u8 =
                      mk_u8 4 +! (if hoist38 >. mk_u64 4 <: bool then mk_u8 0 else mk_u8 3)
                    in
                    (match x <: Core.Option.t_Option u8 with
                      | Core.Option.Option_Some hoist50 ->
                        (match y <: Core.Option.t_Option u8 with
              ...
              | _ ->
                ...
                | Core.Option.Option_Some hoist50 ->
                  (match y <: Core.Option.t_Option u8 with
                    | Core.Option.Option_Some hoist51 ->
                      Core.Option.Option_Some
                      (Core.Num.impl_u8__wrapping_add (Core.Num.impl_u8__wrapping_add v hoist50
                            <:
                            u8)
                          hoist51)
                      <:
      ...
      (match x <: Core.Option.t_Option u8 with
        | Core.Option.Option_Some hoist46 ->
          (match y <: Core.Option.t_Option u8 with
            | Core.Option.Option_Some hoist45 ->
              (match
                  Core.Option.Option_Some (Core.Num.impl_u8__wrapping_add hoist46 hoist45)
                  <:
                  Core.Option.t_Option u8
                with
                | Core.Option.Option_Some hoist49 ->
...
let monad_lifting (x: u8) : Core.Result.t_Result t_A t_B =
  ...
  then
    ...
    | Core.Result.Result_Ok hoist55 -> Core.Result.Result_Ok hoist55 <: Core.Result.t_Result t_A t_B
