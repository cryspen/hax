module Tests.Legacy__mut_ref_functionalization
...
let f (_: Prims.unit) : Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
  ...
  in
  let vec:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
    Core_models.Slice.impl__swap #u8 vec (mk_usize 0) (mk_usize 1)
  in
  let vec:Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
    Core_models.Slice.impl__swap #u8 vec (mk_usize 0) (mk_usize 1)
  in
...
let g (x: t_Pair (Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global))
  ...
  in
  let x:t_Pair (Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global) =
    { x with f_a = Core_models.Slice.impl__swap #u8 x.f_a (mk_usize 0) (mk_usize 1) }
    <:
    t_Pair (Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global)
  in
  let x:t_Pair (Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global) =
    {
      ...
      {
        x.f_b with
        f_field = Core_models.Slice.impl__swap #u8 x.f_b.f_field (mk_usize 0) (mk_usize 1)
      }
      <:
      t_Foo
    }
