...
def Tests.Legacy__proverif_noise__lib.Noise_crypto.DHLEN : usize :=
  Result.of_isOk (do (pure (32 : usize))) (by rfl)
...
def Tests.Legacy__proverif_noise__lib.Noise_crypto.generate_keypair
  ...
  (pure (Tests.Legacy__proverif_noise__lib.Noise_crypto.KeyPair.mk
...
def Tests.Legacy__proverif_noise__lib.Noise_crypto.dh
  ...
  (Hax_lib_protocol.Crypto.dh_scalar_multiply
    ...
    (← (Core.Clone.Clone.clone
      (Tests.Legacy__proverif_noise__lib.Noise_crypto.KeyPair.private_key sk)))
    pk)
...
def Tests.Legacy__proverif_noise__lib.Noise_crypto.decrypt
  ...
  (Core.Result.Impl.map_err
    ...
    (fun _ => (do
      (pure Tests.Legacy__proverif_noise__lib.Noise_crypto.Error.CryptoError) :
      Result Tests.Legacy__proverif_noise__lib.Noise_crypto.Error)))
...
def Tests.Legacy__proverif_noise__lib.Noise_crypto.HASHLEN : usize :=
  Result.of_isOk (do (pure (32 : usize))) (by rfl)

def Tests.Legacy__proverif_noise__lib.Noise_crypto.BLOCKLEN : usize :=
  Result.of_isOk (do (pure (64 : usize))) (by rfl)
...
def Tests.Legacy__proverif_noise__lib.Noise_crypto.hkdf1
  ...
  : Result (Alloc.Vec.Vec u8 Alloc.Alloc.Global)
  := do
  let secret : (Alloc.Vec.Vec u8 Alloc.Alloc.Global) ←
...
def Tests.Legacy__proverif_noise__lib.Noise_lib.initialize_key
  ...
  := do
  (pure (Tests.Legacy__proverif_noise__lib.Noise_lib.CipherState.mk
    (k := key) (n := (0 : u64))))
...
def Tests.Legacy__proverif_noise__lib.Noise_lib.initialize_symmetric
  ...
  := do
  let pnlen : usize ← (Core.Slice.Impl.len u8 protocol_name);
  let hv : (Alloc.Vec.Vec u8 Alloc.Alloc.Global) ←
    if
    (← (Rust_primitives.Hax.Machine_int.lt
      pnlen
      Tests.Legacy__proverif_noise__lib.Noise_crypto.HASHLEN)) then
      (Alloc.Slice.Impl.concat (RustSlice u8) u8
        (← (Rust_primitives.unsize
          #v[protocol_name,
               (← (Core.Ops.Deref.Deref.deref
  ...
  let ck : (Alloc.Vec.Vec u8 Alloc.Alloc.Global) ← (Core.Clone.Clone.clone hv);
  (pure (Tests.Legacy__proverif_noise__lib.Noise_lib.SymmetricState.mk
...
def
  ...
  : Tests.Legacy__proverif_noise__lib.Noise_kkpsk0.ProtocolName
  :=
  Result.of_isOk
    (do
    (pure (Tests.Legacy__proverif_noise__lib.Noise_kkpsk0.ProtocolName.mk
      #v[(78 : u8),
           (111 : u8),
           (105 : u8),
           (115 : u8),
...
def Tests.Legacy__proverif_noise__lib.Noise_kkpsk0.initialize_initiator
  ...
  := do
  let st : Tests.Legacy__proverif_noise__lib.Noise_lib.SymmetricState ←
  ...
  (pure (Tests.Legacy__proverif_noise__lib.Noise_kkpsk0.HandshakeStateI0.mk
    (psk := psk)
    (st := st)
    (s := s)
    (e := e)
  ...
  let st : Tests.Legacy__proverif_noise__lib.Noise_lib.SymmetricState ←
    (Tests.Legacy__proverif_noise__lib.Noise_lib.mix_hash
      ...
      (← (Core.Ops.Deref.Deref.deref
        (Tests.Legacy__proverif_noise__lib.Noise_crypto.KeyPair.public_key
          s))));
  (pure (Tests.Legacy__proverif_noise__lib.Noise_kkpsk0.HandshakeStateR0.mk
    (st := st)
    (psk := psk)
    (s := s)
    (e := e)
...
def Tests.Legacy__proverif_noise__lib.Noise_kkpsk0.write_message1
  ...
  match
    (← (Tests.Legacy__proverif_noise__lib.Noise_lib.encrypt_and_hash
      st
      payload))
  with
