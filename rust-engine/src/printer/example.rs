...
const _: () = {
    ...
    macro_rules! sep {
        ($l:expr, ..$iterable:expr, $r:expr, $sep:expr$(,)?) => {
            sep!($l, iter_pretty!($iterable), $r, $sep)
        };
    ...
    macro_rules! sep_opt {
        ...
        ($l:expr, ..$iterable:expr, $($rest:tt)*) => {
            sep_opt!(@$l, iter_pretty!($iterable), $($rest)*)
        };
        ($l:expr, $it:expr, $($rest:tt)*) => {
            sep_opt!(@$l, $it, $($rest)*)
        };
    ...
    #[extension_traits::extension(trait LocalHelper)]
    impl<'a, 'b, A: 'a + Clone> Allocator<Rust>
    where
        Self: DocAllocator<'a, A> + Printer,
        <Self as DocAllocator<'a, A>>::Doc: Clone,
    {
        fn generic_params(
            &'a self,
        ...
        fn where_clause(
            ...
            docs![
                ...
                "where",
                line!(),
                intersperse!(iter_pretty!(constraints), docs![",", line!()])
                    .nest(INDENT)
                    .group(),
                line!(),
            ]
        ...
        fn attributes(&'a self, attrs: &'b [Attribute]) -> pretty::DocBuilder<'a, Self, A> {
            concat!(
                attrs
                    .iter()
    ...
    impl<'a, 'b, A: 'a + Clone> PrettyAst<'a, 'b, A> for Allocator<Rust> {
        ...
        fn pat(&'a self, pat: &'b Pat) -> pretty::DocBuilder<'a, Self, A> {
            match &*pat.kind {
                ...
                PatKind::Or { sub_pats } => {
                    intersperse!(iter_pretty!(sub_pats), docs![line!(), "|", line!()])
                }
                ...
                PatKind::Binding {
                    ...
                    docs![
                        ...
                        mode,
                        var,
                        sub_pat.as_ref().map(|pat| docs!["@", p!(pat)]),
                    ]
                }
                PatKind::Construct {
                    constructor,
        ...
        fn ty(&'a self, ty: &'b Ty) -> pretty::DocBuilder<'a, Self, A> {
            match ty.kind() {
                ...
                TyKind::Tuple(items) => intersperse!(iter_pretty!(items), docs![",", line!()])
                    .nest(INDENT)
                    .group(),
        ...
        fn literal(&'a self, literal: &'b literals::Literal) -> pretty::DocBuilder<'a, Self, A> {
            match literal {
                ...
                literals::Literal::Int {
                    ...
                    negative,
                    kind,
                } => docs![if *negative { p!("-") } else { nil!() }, value, kind],
        ...
        fn expr(&'a self, expr: &'b Expr) -> pretty::DocBuilder<'a, Self, A> {
            match &*expr.kind {
                ExprKind::If {
                    ...
                    p!(then).braces(),
                    "else",
                    opt!(else_).map(|doc| doc.braces()).unwrap_or(nil!())
                ],
                ExprKind::App {
                    head,
                    args,
                ...
                ExprKind::Closure {
                    ...
                    intersperse!(iter_pretty!(params), docs![",", space!()]).enclose("|", "|"),
                    body
                ],
        ...
        fn lhs(&'a self, lhs: &'b Lhs) -> pretty::DocBuilder<'a, Self, A> {
            match lhs {
                ...
                Lhs::ArrayAccessor { e, ty: _, index } => {
                    docs![std::ops::Deref::deref(e), p!(index).enclose("[", "]")]
                }
            }
        }
        fn global_id(
