...
/// The inputs a proof scenario resolves for the F* backend, carried through
/// the `__json` re-entry rather than argv. Empty on flag-driven `into`
/// invocations.
#[derive_group(Serializers)]
...
pub struct FStarScenarioOptions {
    ...
    /// The command that reproduces the extraction, for the generated
    /// `Makefile`. A scenario re-enters through `__json`, so the argv of
    /// the run that writes the file does not spell its own invocation.
...
mod tests {
    ...
    /// `scenario` is not a flag: it travels through `__json`.
    #[test]
