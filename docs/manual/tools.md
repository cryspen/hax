...
- F\*: `z3rlimit`, `fuel`, `ifuel`, `interfaces`, `line-width`, and `project-files`. The first five mirror flags of `cargo hax into fstar`; see its `--help` for their effects. The per-scenario `project-files` key overrides the top-level one.
