...
mod prelude {
    ...
    pub use crate::unified_macros::HaxMacroInput;
...
pub fn impl_fn_decoration(attr: pm::TokenStream, item: pm::TokenStream) -> pm::TokenStream {
    ...
    let (decoration, attr) =
        make_fn_decoration(phi, item.sig.clone(), kind, generics, Some(self_ty));
            ...
            for ti in item.items.iter_mut() {
                if let TraitItem::Fn(fun) = ti {
                    for attr in &mut fun.attrs {
                        ...
                        let (decoration, relation_attr) =
                            make_fn_decoration(phi, fun.sig.clone(), kind, generics, Some(self_ty));
...
pub fn refinement_type(mut attr: pm::TokenStream, item: pm::TokenStream) -> pm::TokenStream {
    ...
    let (refinement_item, refinement_attr) =
        make_fn_decoration(phi.clone(), sig, kind, Generics::default(), None);
