...
Record Positive_Positive_record : Type :=
  {
    Positive_Positive_0 : hax_core.base.spec.haxint.t_HaxInt;
  }.
...
Instance core.clone.t_Clone_520604619 : core.clone.t_Clone ((t_Positive)) :=
  {
...
Definition positive_to_int (s : t_Positive) : hax_core.base.spec.haxint.t_HaxInt :=
  0 s.

Definition positive_from_int (x : hax_core.base.spec.haxint.t_HaxInt) : t_Positive :=
  Positive_Positive (x).

Definition xH : t_Positive :=
  Positive_Positive (hax_core.base.spec.haxint.v_HaxInt_ONE).
...
Definition match_positive (s : t_Positive) : t_POSITIVE :=
  if
    match_positive__is_xH (core.clone.Clone__f_clone (s))
  then
    POSITIVE_XH
  else
    if
      match_positive__is_xO (core.clone.Clone__f_clone (s))
    then
      POSITIVE_XO (positive_from_int (hax_core.base.spec.haxint.div2 (positive_to_int (s))))
    else
      POSITIVE_XI (positive_from_int (hax_core.base.spec.haxint.div2 (positive_to_int (s)))).
