...
local macro "uconcretize_spec " fn:ident T:ident : command => do
  ...
  `(@[spec] theorem $specName {Q} {n : Int}
      (hok : ∀ r : $T, (r.val : Int) = n → willYield r Q)
      (h : (n < 0 ∨ n > $maxId) → willFail .integerOverflow Q) :
      ⦃ ⌜ True ⌝ ⦄ $fn n ⦃ Q ⦄ := by
    mvcgen [$fn:term, UScalar.tryMk, UScalar.tryMkOpt, ofOption] <;> grind)
...
local macro "iconcretize_spec " fn:ident T:ident : command => do
  ...
  let maxId := mkIdent (T.getId ++ `max)
  `(@[spec] theorem $specName {Q} {n : Int}
      (hok : ∀ r : $T, r.val = n → willYield r Q)
      (h : (n < $minId ∨ n > $maxId) → willFail .integerOverflow Q) :
      ⦃ ⌜ True ⌝ ⦄ $fn n ⦃ Q ⦄ := by
    mvcgen [$fn:term, IScalar.tryMk, IScalar.tryMkOpt, ofOption] <;> grind)
