...
include Core_models.Result_Option_bundle {t_Option, impl__ok_or, impl__ok_or_else}
...
let impl__unwrap_or_default 
      #t {| i1: Core_models.Default.t_Default t |}
      (self: t_Option t)
      =
  match self with
  | Option_Some inner -> inner
  | Core_models.Option.Option_None  -> Core_models.Default.f_default #t ()
...
let impl__unwrap_or
  ...
  match self with
  | Option_Some inner -> inner
  | Core_models.Option.Option_None  -> def
