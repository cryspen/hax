...
let impl_usize__checked_sub (x: usize) (y: usize): Core.Option.t_Option usize =
  if x >=. y then Core.Option.Option_Some (x -! y) else Core.Option.Option_None

let impl_usize__checked_add (x: usize) (y: usize): Core.Option.t_Option usize =
  if range (v x + v y) USIZE then Core.Option.Option_Some (x +! y) else Core.Option.Option_None
