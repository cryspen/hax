...
class t_Read (v_Self: Type0) = {
  ...
  f_read_post:v_Self -> x: t_Slice u8 -> (v_Self & t_Array u8 (Core.Slice.impl__len #u8 x <: usize) & Core.Result.t_Result usize Std.Io.Error.t_Error)
    -> bool;
  f_read:x0: v_Self -> x1: t_Slice u8
    -> Prims.Pure (v_Self & t_Array u8 (Core.Slice.impl__len #u8 x1 <: usize) & Core.Result.t_Result usize Std.Io.Error.t_Error)
        (f_read_pre x0 x1)
        (fun result -> f_read_post x0 x1 result);
  f_read_exact_pre:v_Self -> t_Slice u8 -> r:bool{true ==> r};
  f_read_exact_post:v_Self -> x: t_Slice u8 -> res: (v_Self & t_Array u8 (Core.Slice.impl__len #u8 x <: usize) & Core.Result.t_Result unit Std.Io.Error.t_Error)
    -> bool;
  f_read_exact: x0: v_Self -> x1: t_Slice u8
    -> Prims.Pure (v_Self & t_Array u8 (Core.Slice.impl__len #u8 x1 <: usize) & Core.Result.t_Result unit Std.Io.Error.t_Error)
