...
class t_FromResidual (v_Self: Type0) (v_R: Type0) = {
  f_from_residual_pre:v_R -> prop;
  f_from_residual_post:v_R -> v_Self -> prop;
...
class t_Try (v_Self: Type0) = {
  ...
  f_from_output_pre:f_Output -> prop;
  f_from_output_post:f_Output -> v_Self -> prop;
  f_from_output:x0: f_Output
    -> Prims.Pure v_Self (f_from_output_pre x0) (fun result -> f_from_output_post x0 result);
  f_branch_pre:v_Self -> prop;
  f_branch_post:v_Self -> Core_models.Ops.Control_flow.t_ControlFlow f_Residual f_Output -> prop;
  f_branch:x0: v_Self
