...
(* list_init is defined as let rec in .fsti for normalization *)
...
let list_init_index (#a: Type) (n: nat) (f: (i:nat{i < n}) -> a) (k: nat{k < n}):
  ...
  = admit () // list_init is let rec in .fsti but opaque to proofs in .fst
...
(* list_upd is defined as let rec in .fsti for normalization *)
...
let list_upd_index (#a: Type) (l: list a) (i: nat{i < FStar.List.Tot.length l}) (x: a) (j: nat{j < FStar.List.Tot.length l}):
  ...
  = admit () // list_upd is let rec in .fsti but opaque to proofs in .fst
...
private let rec eq_intro_aux (#t: Type) (a b: list t):
  Lemma (requires FStar.List.Tot.length a == FStar.List.Tot.length b /\
                  (forall (i:nat). i < FStar.List.Tot.length a ==>
                       FStar.List.Tot.index a i == FStar.List.Tot.index b i))
        (ensures a == b)
        (decreases (FStar.List.Tot.length a))
  = match a, b with
    | [], [] -> ()
    | ha :: ta, hb :: tb ->
      assert (FStar.List.Tot.index a 0 == ha);
      assert (FStar.List.Tot.index b 0 == hb);
      let aux (i: nat{i < FStar.List.Tot.length ta}):
        Lemma (FStar.List.Tot.index ta i == FStar.List.Tot.index tb i)
        = assert (FStar.List.Tot.index a (i + 1) == FStar.List.Tot.index ta i);
          assert (FStar.List.Tot.index b (i + 1) == FStar.List.Tot.index tb i)
      in
...
let lemma_slice_append_3 #t (x:t_Slice t) (y:t_Slice t) (z:t_Slice t) (w:t_Slice t):
  ...
  = let zw = list_append z w in
    let yzw = list_append y zw in
    eq_intro x yzw
...
let lemma_slice_append_4 #t (x y z w u:t_Slice t) :
  ...
  = let wu = list_append w u in
    let zwu = list_append z wu in
    let yzwu = list_append y zwu in
    eq_intro x yzwu
#pop-options
