...
mod testing {
    ...
    /// `PartialEq` has to agree with `Ord`, which rules out deriving it over
    /// both fields.
    ...
    impl PartialEq for Tagged {
        // Excluded from coverage: this is only here to satisfy `Ord`'s
        // supertrait bound, and the tests compare tags rather than `Tagged`s.
