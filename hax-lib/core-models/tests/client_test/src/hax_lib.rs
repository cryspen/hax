...
//! The specs need only type-check — they are not meant to be true.
...
#[hax_lib::ensures(|r| r.to_int() == a.to_int() + b.to_int() - a.to_int() * b.to_int() / int!(1))]
fn int_arith(a: u32, b: u32) -> u32 {
    a
}

#[hax_lib::ensures(|r| -(a.to_int()) == int!(0) - a.to_int())]
fn int_neg(a: i32) -> i32 {
    a
}

// --- inherent methods: rem_euclid, pow2, and the `int!` literal macro ---
#[hax_lib::ensures(|r| r.to_int() == a.to_int().rem_euclid(b.to_int()))]
fn int_rem_euclid(a: u32, b: u32) -> u32 {
    a
}

#[hax_lib::ensures(|r| r.to_int() == int!(2).pow2() + int!(0x2a))]
...
#[hax_lib::ensures(|r| a.to_int().cmp(&b.to_int()).is_eq() || true)]
