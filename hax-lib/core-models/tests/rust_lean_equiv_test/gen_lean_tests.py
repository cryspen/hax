...
def render(qualified: list[str], preamble: list[str]) -> str:
    lines = [
...
def generate() -> None:
    ...
    for name, reason in collect_source_tests():
        ...
        if reason is not None:
            (skipped if fq else skipped_unextracted).append(fq or name)
        elif fq:
            active.append(fq)
        else:
            missing.append(name)
...
def report_skipped(log_path: pathlib.Path) -> None:
    ...
    guards = re.findall(r"^#guard (\S+) ==", OUT_SKIPPED.read_text(), re.MULTILINE)
    if not guards:
        print("gen_lean_tests.py: no skipped tests.")
        return
    log = log_path.read_text()
    passing = [g for g in guards if g not in log]
    if passing:
        listing = "\n".join(f"  - {g}" for g in passing)
        sys.exit(
