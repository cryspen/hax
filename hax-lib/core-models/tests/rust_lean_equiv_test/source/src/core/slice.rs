...
// Rust-only: `&[T] == [U; N]` resolves to
// `core.Shared0Slice.Insts.CoreCmpPartialEqArray.eq`, which the model publishes
// as `core.Slice.Insts.…`. `skip_lean` cannot apply -- the guard fails to
// elaborate, not to hold.
#[cfg(test)]
#[test]
fn test_slice_array_eq_goes_through_the_dictionary() {
    let a = [keyed(1, 1), keyed(2, 1)];
    let b = [keyed(1, 2), keyed(2, 2)];
    assert!(a.as_slice() == b);
}
