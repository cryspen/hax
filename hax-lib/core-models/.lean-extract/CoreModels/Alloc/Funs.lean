-- THIS FILE WAS AUTOMATICALLY GENERATED BY AENEAS
-- [alloc_models]: function definitions
import Aeneas
import CoreModels
import CoreModels.Alloc.Types
import CoreModels.Alloc.FunsExternal
open CoreModels Aeneas
open Aeneas.Std hiding namespace core alloc
open Result ControlFlow Error
open Std.Do
set_option linter.dupNamespace false
set_option linter.hashCommand false
set_option linter.unusedVariables false

/- You can set the `maxHeartbeats` value with the `-max-heartbeats` CLI option -/
set_option maxHeartbeats 1000000

/- You can set the `maxRecDepth` value with the `-max-recdepth` CLI option -/
set_option maxRecDepth 2048

namespace alloc_models

/-- [alloc_models::alloc::{impl core::clone::Clone for alloc_models::alloc::Global}::clone]:
    Source: 'src/lib.rs', lines 28:13-28:18
    Visibility: public -/
def alloc.Global.Insts.CoreCloneClone.clone
  (self : alloc.Global) : Result alloc.Global := do
  ok ()

/-- Trait implementation: [alloc_models::alloc::{impl core::clone::Clone for alloc_models::alloc::Global}]
    Source: 'src/lib.rs', lines 28:13-28:18 -/
@[reducible]
def alloc.Global.Insts.CoreCloneClone : core.clone.Clone alloc.Global := {
  clone := alloc.Global.Insts.CoreCloneClone.clone
}

/-- Trait implementation: [alloc_models::alloc::{impl alloc_models::alloc::Allocator for alloc_models::alloc::Global}]
    Source: 'src/lib.rs', lines 31:4-31:32 -/
@[reducible]
def alloc.Global.Insts.Alloc_modelsAllocAllocator : alloc.Allocator
  alloc.Global := {
}

/-- [alloc_models::borrow::{impl alloc_models::borrow::ToOwned for T}::to_owned]:
    Source: 'src/lib.rs', lines 41:8-43:9
    Visibility: public -/
def borrow.ToOwned.Blanket.to_owned {T : Type} (self : T) : Result T := do
  ok self

/-- Trait implementation: [alloc_models::borrow::{impl alloc_models::borrow::ToOwned for T}]
    Source: 'src/lib.rs', lines 40:4-44:5 -/
@[reducible]
def borrow.ToOwned.Blanket (T : Type) : borrow.ToOwned T := {
  to_owned := borrow.ToOwned.Blanket.to_owned
}

/-- [alloc_models::boxed::{alloc_models::boxed::Box<T>}::new]:
    Source: 'src/lib.rs', lines 63:8-65:9 -/
def boxed.Box.new {T : Type} (v : T) : Result T := do
  ok v

/-- [alloc_models::collections::btree::set::{alloc_models::collections::btree::set::BTreeSet<T, U>}::new]:
    Source: 'src/lib.rs', lines 225:16-227:17 -/
def collections.btree.set.BTreeSet.new
  (T : Type) (U : Type) : Result (collections.btree.set.BTreeSet T U) := do
  ok (core.option.Option.None, core.option.Option.None)

/-- [alloc_models::collections::vec_deque::{alloc_models::collections::vec_deque::VecDeque<T, alloc_models::alloc::Global>}::new]:
    Source: 'src/lib.rs', lines 256:12-258:13 -/
def collections.vec_deque.VecDequeTGlobal.new
  (T : Type) : Result (collections.vec_deque.VecDeque T alloc.Global) := do
  let s ← rust_primitives.sequence.seq_empty T
  ok (s, ())

/-- [alloc_models::collections::vec_deque::{alloc_models::collections::vec_deque::VecDeque<T, alloc_models::alloc::Global>}::with_capacity]:
    Source: 'src/lib.rs', lines 259:12-261:13 -/
def collections.vec_deque.VecDequeTGlobal.with_capacity
  (T : Type) (_capacity : Std.Usize) :
  Result (collections.vec_deque.VecDeque T alloc.Global)
  := do
  collections.vec_deque.VecDequeTGlobal.new T

/-- [alloc_models::collections::vec_deque::{alloc_models::collections::vec_deque::VecDeque<T, A>}::push_back]:
    Source: 'src/lib.rs', lines 267:12-269:13 -/
def collections.vec_deque.VecDeque.push_back
  {T : Type} {A : Type} (self : collections.vec_deque.VecDeque T A) (x : T) :
  Result (collections.vec_deque.VecDeque T A)
  := do
  let (s, pd) := self
  let s1 ← rust_primitives.sequence.seq_push s x
  ok (s1, pd)

/-- [alloc_models::collections::vec_deque::{alloc_models::collections::vec_deque::VecDeque<T, A>}::len]:
    Source: 'src/lib.rs', lines 270:12-272:13 -/
def collections.vec_deque.VecDeque.len
  {T : Type} {A : Type} (self : collections.vec_deque.VecDeque T A) :
  Result Std.Usize
  := do
  let (s, _) := self
  rust_primitives.sequence.seq_len s

/-- [alloc_models::collections::vec_deque::{alloc_models::collections::vec_deque::VecDeque<T, A>}::pop_front]:
    Source: 'src/lib.rs', lines 273:12-279:13 -/
def collections.vec_deque.VecDeque.pop_front
  {T : Type} {A : Type} (self : collections.vec_deque.VecDeque T A) :
  Result ((core.option.Option T) × (collections.vec_deque.VecDeque T A))
  := do
  let i ← collections.vec_deque.VecDeque.len self
  if i = 0#usize
  then ok (core.option.Option.None, self)
  else
    let (s, pd) := self
    let (t, s1) ← rust_primitives.sequence.seq_remove s 0#usize
    ok (core.option.Option.Some t, (s1, pd))

/-- [alloc_models::collections::vec_deque::{impl core::ops::index::Index<usize, T> for alloc_models::collections::vec_deque::VecDeque<T, A>}::index]:
    Source: 'src/lib.rs', lines 286:12-288:13
    Visibility: public -/
def collections.vec_deque.VecDeque.Insts.CoreOpsIndexIndexUsizeT.index
  {T : Type} {A : Type} (self : collections.vec_deque.VecDeque T A)
  (i : Std.Usize) :
  Result T
  := do
  let (s, _) := self
  rust_primitives.sequence.seq_index s i

/-- Trait implementation: [alloc_models::collections::vec_deque::{impl core::ops::index::Index<usize, T> for alloc_models::collections::vec_deque::VecDeque<T, A>}]
    Source: 'src/lib.rs', lines 283:8-289:9 -/
@[reducible]
def collections.vec_deque.VecDeque.Insts.CoreOpsIndexIndexUsizeT (T : Type) (A
  : Type) : core.ops.index.Index (collections.vec_deque.VecDeque T A) Std.Usize
  T := {
  index := collections.vec_deque.VecDeque.Insts.CoreOpsIndexIndexUsizeT.index
}

/-- [alloc_models::collections::vec_deque::into_iter::{impl core::iter::traits::iterator::Iterator<T> for alloc_models::collections::vec_deque::into_iter::IntoIter<T, A>}::next]:
    Source: 'src/lib.rs', lines 296:16-302:17
    Visibility: public -/
def
  collections.vec_deque.into_iter.IntoIter.Insts.CoreIterTraitsIteratorIterator.next
  {T : Type} {A : Type} (self : collections.vec_deque.into_iter.IntoIter T A) :
  Result ((core.option.Option T) × (collections.vec_deque.into_iter.IntoIter T
    A))
  := do
  let (s, pd) := self
  let i ← rust_primitives.sequence.seq_len s
  if i = 0#usize
  then ok (core.option.Option.None, self)
  else
    let (t, s1) ← rust_primitives.sequence.seq_remove s 0#usize
    ok (core.option.Option.Some t, (s1, pd))

/-- Trait implementation: [alloc_models::collections::vec_deque::into_iter::{impl core::iter::traits::iterator::Iterator<T> for alloc_models::collections::vec_deque::into_iter::IntoIter<T, A>}]
    Source: 'src/lib.rs', lines 294:12-303:13 -/
@[reducible]
def
  collections.vec_deque.into_iter.IntoIter.Insts.CoreIterTraitsIteratorIterator
  (T : Type) (A : Type) : core.iter.traits.iterator.Iterator
  (collections.vec_deque.into_iter.IntoIter T A) T := {
  next :=
    collections.vec_deque.into_iter.IntoIter.Insts.CoreIterTraitsIteratorIterator.next
}

/-- [alloc_models::collections::vec_deque::{impl core::iter::traits::collect::IntoIterator<T, alloc_models::collections::vec_deque::into_iter::IntoIter<T, A>> for alloc_models::collections::vec_deque::VecDeque<T, A>}::into_iter]:
    Source: 'src/lib.rs', lines 310:12-312:13
    Visibility: public -/
def
  collections.vec_deque.VecDeque.Insts.CoreIterTraitsCollectIntoIteratorTIntoIter.into_iter
  {T : Type} {A : Type} (self : collections.vec_deque.VecDeque T A) :
  Result (collections.vec_deque.into_iter.IntoIter T A)
  := do
  let (s, _) := self
  ok (s, ())

/-- Trait implementation: [alloc_models::collections::vec_deque::{impl core::iter::traits::collect::IntoIterator<T, alloc_models::collections::vec_deque::into_iter::IntoIter<T, A>> for alloc_models::collections::vec_deque::VecDeque<T, A>}]
    Source: 'src/lib.rs', lines 307:8-313:9 -/
@[reducible]
def
  collections.vec_deque.VecDeque.Insts.CoreIterTraitsCollectIntoIteratorTIntoIter
  (T : Type) (A : Type) : core.iter.traits.collect.IntoIterator
  (collections.vec_deque.VecDeque T A) T
  (collections.vec_deque.into_iter.IntoIter T A) := {
  iteratorIteratorInst :=
    collections.vec_deque.into_iter.IntoIter.Insts.CoreIterTraitsIteratorIterator
    T A
  into_iter :=
    collections.vec_deque.VecDeque.Insts.CoreIterTraitsCollectIntoIteratorTIntoIter.into_iter
}

/-- [alloc_models::vec::from_seq]:
    Source: 'src/lib.rs', lines 668:4-670:5 -/
def vec.from_seq
  {T : Type} (s : rust_primitives.sequence.Seq T) : Result (vec.Vec T) := do
  ok s

/-- [alloc_models::slice::{alloc_models::slice::Dummy<T>}::to_vec]:
    Source: 'src/lib.rs', lines 451:8-458:9 -/
def slice.Dummy.to_vec
  {T : Type} (corecloneCloneInst : core.clone.Clone T) (s : Slice T) :
  Result (vec.Vec T)
  := do
  let seq ← rust_primitives.sequence.seq_empty T
  let seq1 ← rust_primitives.sequence.seq_extend corecloneCloneInst seq s
  vec.from_seq seq1

/-- [alloc_models::slice::{alloc_models::slice::Dummy<T>}::into_vec]:
    Source: 'src/lib.rs', lines 460:8-462:9 -/
def slice.Dummy.into_vec {T : Type} (s : Slice T) : Result (vec.Vec T) := do
  let s1 ← rust_primitives.sequence.seq_from_boxed_slice s
  vec.from_seq s1

/-- [alloc_models::slice::{alloc_models::slice::Dummy<T>}::concat]: loop body 0:
    Source: 'src/lib.rs', lines 475:12-478:13 -/
@[rust_loop_body]
def slice.Dummy.concat_loop.body
  {T : Type} {Item : Type} (corecloneCloneInst : core.clone.Clone Item)
  (coreborrowBorrowTSliceInst : core.borrow.Borrow T (Slice Item))
  (s : Slice T) (out : rust_primitives.sequence.Seq Item) (i : Std.Usize) :
  Result (ControlFlow ((rust_primitives.sequence.Seq Item) × Std.Usize)
    (rust_primitives.sequence.Seq Item))
  := do
  let i1 ← rust_primitives.slice.slice_length s
  if i < i1
  then
    let t ← rust_primitives.slice.slice_index s i
    let s1 ← coreborrowBorrowTSliceInst.borrow t
    let out1 ← rust_primitives.sequence.seq_extend corecloneCloneInst out s1
    let i2 ← i + 1#usize
    ok (cont (out1, i2))
  else ok (done out)

/-- [alloc_models::slice::{alloc_models::slice::Dummy<T>}::concat]: loop 0:
    Source: 'src/lib.rs', lines 475:12-478:13 -/
@[rust_loop]
def slice.Dummy.concat_loop
  {T : Type} {Item : Type} (corecloneCloneInst : core.clone.Clone Item)
  (coreborrowBorrowTSliceInst : core.borrow.Borrow T (Slice Item))
  (s : Slice T) (out : rust_primitives.sequence.Seq Item) (i : Std.Usize) :
  Result (rust_primitives.sequence.Seq Item)
  := do
  loop
    (fun (out1, i1) => slice.Dummy.concat_loop.body corecloneCloneInst
      coreborrowBorrowTSliceInst s out1 i1)
    (out, i)

/-- [alloc_models::slice::{alloc_models::slice::Dummy<T>}::concat]:
    Source: 'src/lib.rs', lines 469:8-480:9 -/
def slice.Dummy.concat
  {T : Type} {Item : Type} (corecloneCloneInst : core.clone.Clone Item)
  (coreborrowBorrowTSliceInst : core.borrow.Borrow T (Slice Item))
  (s : Slice T) :
  Result (vec.Vec Item)
  := do
  let out ← rust_primitives.sequence.seq_empty Item
  let out1 ←
    slice.Dummy.concat_loop corecloneCloneInst coreborrowBorrowTSliceInst s out
      0#usize
  vec.from_seq out1

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::as_slice]:
    Source: 'src/lib.rs', lines 783:8-785:9
    Visibility: public -/
def vec.Vec.as_slice {T : Type} (self : vec.Vec T) : Result (Slice T) := do
  rust_primitives.sequence.seq_to_slice self

/-- [alloc_models::vec::{impl core::ops::deref::Deref<[T]> for alloc_models::vec::Vec<T>}::deref]:
    Source: 'src/lib.rs', lines 930:8-932:9
    Visibility: public -/
def vec.Vec.Insts.CoreOpsDerefDerefSlice.deref
  {T : Type} (self : vec.Vec T) : Result (Slice T) := do
  vec.Vec.as_slice self

/-- [alloc_models::vec::{impl core::clone::Clone for alloc_models::vec::Vec<T>}::clone]: loop body 0:
    Source: 'src/lib.rs', lines 675:12-677:13
    Visibility: public -/
@[rust_loop_body]
def vec.Vec.Insts.CoreCloneClone.clone_loop.body
  {T : Type} (corecloneCloneInst : core.clone.Clone T)
  (iter : core.slice.iter.Iter T) (new_vec : rust_primitives.sequence.Seq T) :
  Result (ControlFlow ((core.slice.iter.Iter T) ×
    (rust_primitives.sequence.Seq T)) (rust_primitives.sequence.Seq T))
  := do
  let (o, iter1) ←
    core.slice.iter.Iter.Insts.CoreIterTraitsIteratorIteratorSharedAT.next iter
  match o with
  | core.option.Option.None => ok (done new_vec)
  | core.option.Option.Some it =>
    let t ← corecloneCloneInst.clone it
    let new_vec1 ← rust_primitives.sequence.seq_push new_vec t
    ok (cont (iter1, new_vec1))

/-- [alloc_models::vec::{impl core::clone::Clone for alloc_models::vec::Vec<T>}::clone]: loop 0:
    Source: 'src/lib.rs', lines 675:12-677:13
    Visibility: public -/
@[rust_loop]
def vec.Vec.Insts.CoreCloneClone.clone_loop
  {T : Type} (corecloneCloneInst : core.clone.Clone T)
  (iter : core.slice.iter.Iter T) (new_vec : rust_primitives.sequence.Seq T) :
  Result (rust_primitives.sequence.Seq T)
  := do
  loop
    (fun (iter1, new_vec1) => vec.Vec.Insts.CoreCloneClone.clone_loop.body
      corecloneCloneInst iter1 new_vec1)
    (iter, new_vec)

/-- [alloc_models::vec::{impl core::clone::Clone for alloc_models::vec::Vec<T>}::clone]:
    Source: 'src/lib.rs', lines 673:8-679:9
    Visibility: public -/
def vec.Vec.Insts.CoreCloneClone.clone
  {T : Type} (corecloneCloneInst : core.clone.Clone T) (self : vec.Vec T) :
  Result (vec.Vec T)
  := do
  let new_vec ← rust_primitives.sequence.seq_empty T
  let s ← vec.Vec.Insts.CoreOpsDerefDerefSlice.deref self
  let iter ← core.slice.Slice.iter s
  let new_vec1 ←
    vec.Vec.Insts.CoreCloneClone.clone_loop corecloneCloneInst iter new_vec
  ok new_vec1

/-- Trait implementation: [alloc_models::vec::{impl core::clone::Clone for alloc_models::vec::Vec<T>}]
    Source: 'src/lib.rs', lines 672:4-680:5 -/
@[reducible]
def vec.Vec.Insts.CoreCloneClone {T : Type} (corecloneCloneInst :
  core.clone.Clone T) : core.clone.Clone (vec.Vec T) := {
  clone := vec.Vec.Insts.CoreCloneClone.clone corecloneCloneInst
}

/-- [alloc_models::vec::{impl core::ops::index::Index<I, Clause0_Output> for alloc_models::vec::Vec<T>}::index]:
    Source: 'src/lib.rs', lines 905:8-907:9
    Visibility: public -/
def vec.Vec.Insts.CoreOpsIndexIndex.index
  {T : Type} {I : Type} {Clause0_Output : Type}
  (coresliceindexSliceIndexISliceClause0_OutputInst :
  core.slice.index.SliceIndex I (Slice T) Clause0_Output) (self : vec.Vec T)
  (i : I) :
  Result Clause0_Output
  := do
  let s ← vec.Vec.Insts.CoreOpsDerefDerefSlice.deref self
  core.Slice.Insts.CoreOpsIndexIndex.index
    coresliceindexSliceIndexISliceClause0_OutputInst s i

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::len]:
    Source: 'src/lib.rs', lines 757:8-759:9
    Visibility: public -/
def vec.Vec.len {T : Type} (self : vec.Vec T) : Result Std.Usize := do
  rust_primitives.sequence.seq_len self

/-- [alloc_models::vec::{impl core::cmp::PartialEq<alloc_models::vec::Vec<U>> for alloc_models::vec::Vec<T>}::eq]: loop body 0:
    Source: 'src/lib.rs', lines 1:0-701:17
    Visibility: public -/
@[rust_loop_body]
def vec.Vec.Insts.CoreCmpPartialEqVec.eq_loop.body
  {T : Type} {U : Type} (corecmpPartialEqInst : core.cmp.PartialEq T U)
  (self : vec.Vec T) (other : vec.Vec U)
  (iter : core.ops.range.Range Std.Usize) (res : Bool) :
  Result (ControlFlow ((core.ops.range.Range Std.Usize) × Bool) Bool)
  := do
  let (o, iter1) ←
    core.ops.range.Range.Insts.CoreIterTraitsIteratorIterator.next
      core.Usize.Insts.CoreIterRangeStep iter
  match o with
  | core.option.Option.None => ok (done res)
  | core.option.Option.Some i =>
    if res
    then
      let t ←
        vec.Vec.Insts.CoreOpsIndexIndex.index
          (core.Usize.Insts.CoreSliceIndexSliceIndexSliceT T) self i
      let t1 ←
        vec.Vec.Insts.CoreOpsIndexIndex.index
          (core.Usize.Insts.CoreSliceIndexSliceIndexSliceT U) other i
      let b ← corecmpPartialEqInst.eq t t1
      if b
      then ok (cont (iter1, true))
      else ok (cont (iter1, false))
    else ok (cont (iter1, false))

/-- [alloc_models::vec::{impl core::cmp::PartialEq<alloc_models::vec::Vec<U>> for alloc_models::vec::Vec<T>}::eq]: loop 0:
    Source: 'src/lib.rs', lines 1:0-701:17
    Visibility: public -/
@[rust_loop]
def vec.Vec.Insts.CoreCmpPartialEqVec.eq_loop
  {T : Type} {U : Type} (corecmpPartialEqInst : core.cmp.PartialEq T U)
  (iter : core.ops.range.Range Std.Usize) (self : vec.Vec T)
  (other : vec.Vec U) (res : Bool) :
  Result Bool
  := do
  loop
    (fun (iter1, res1) => vec.Vec.Insts.CoreCmpPartialEqVec.eq_loop.body
      corecmpPartialEqInst self other iter1 res1)
    (iter, res)

/-- [alloc_models::vec::{impl core::cmp::PartialEq<alloc_models::vec::Vec<U>> for alloc_models::vec::Vec<T>}::eq]:
    Source: 'src/lib.rs', lines 689:8-704:9
    Visibility: public -/
def vec.Vec.Insts.CoreCmpPartialEqVec.eq
  {T : Type} {U : Type} (corecmpPartialEqInst : core.cmp.PartialEq T U)
  (self : vec.Vec T) (other : vec.Vec U) :
  Result Bool
  := do
  let i ← vec.Vec.len self
  let i1 ← vec.Vec.len other
  if i = i1
  then
    vec.Vec.Insts.CoreCmpPartialEqVec.eq_loop corecmpPartialEqInst
      { start := 0#usize, «end» := i } self other true
  else ok false

/-- [alloc_models::vec::{impl core::cmp::PartialEq<alloc_models::vec::Vec<U>> for alloc_models::vec::Vec<T>}::ne]:
    Source: 'src/lib.rs', lines 686:8-688:9
    Visibility: public -/
def vec.Vec.Insts.CoreCmpPartialEqVec.ne
  {T : Type} {U : Type} (corecmpPartialEqInst : core.cmp.PartialEq T U)
  (self : vec.Vec T) (other : vec.Vec U) :
  Result Bool
  := do
  let b ←
    vec.Vec.Insts.CoreCmpPartialEqVec.eq corecmpPartialEqInst self other
  ok (b = false)

/-- Trait implementation: [alloc_models::vec::{impl core::cmp::PartialEq<alloc_models::vec::Vec<U>> for alloc_models::vec::Vec<T>}]
    Source: 'src/lib.rs', lines 681:4-705:5 -/
@[reducible]
def vec.Vec.Insts.CoreCmpPartialEqVec {T : Type} {U : Type}
  (corecmpPartialEqInst : core.cmp.PartialEq T U) : core.cmp.PartialEq (vec.Vec
  T) (vec.Vec U) := {
  eq := vec.Vec.Insts.CoreCmpPartialEqVec.eq corecmpPartialEqInst
  ne := vec.Vec.Insts.CoreCmpPartialEqVec.ne corecmpPartialEqInst
}

/-- [alloc_models::vec::into_iter::{impl core::iter::traits::iterator::Iterator<T> for alloc_models::vec::into_iter::IntoIter<T>}::next]:
    Source: 'src/lib.rs', lines 716:12-722:13
    Visibility: public -/
def vec.into_iter.IntoIter.Insts.CoreIterTraitsIteratorIterator.next
  {T : Type} (self : vec.into_iter.IntoIter T) :
  Result ((core.option.Option T) × (vec.into_iter.IntoIter T))
  := do
  let i ← rust_primitives.sequence.seq_len self
  if i = 0#usize
  then ok (core.option.Option.None, self)
  else
    let (t, s) ← rust_primitives.sequence.seq_remove self 0#usize
    ok (core.option.Option.Some t, s)

/-- Trait implementation: [alloc_models::vec::into_iter::{impl core::iter::traits::iterator::Iterator<T> for alloc_models::vec::into_iter::IntoIter<T>}]
    Source: 'src/lib.rs', lines 714:8-723:9 -/
@[reducible]
def vec.into_iter.IntoIter.Insts.CoreIterTraitsIteratorIterator (T : Type) :
  core.iter.traits.iterator.Iterator (vec.into_iter.IntoIter T) T := {
  next := vec.into_iter.IntoIter.Insts.CoreIterTraitsIteratorIterator.next
}

/-- [alloc_models::vec::{impl core::iter::traits::collect::IntoIterator<T, alloc_models::vec::into_iter::IntoIter<T>> for alloc_models::vec::Vec<T>}::into_iter]:
    Source: 'src/lib.rs', lines 729:8-731:9
    Visibility: public -/
def vec.Vec.Insts.CoreIterTraitsCollectIntoIteratorTIntoIter.into_iter
  {T : Type} (self : vec.Vec T) : Result (vec.into_iter.IntoIter T) := do
  ok self

/-- Trait implementation: [alloc_models::vec::{impl core::iter::traits::collect::IntoIterator<T, alloc_models::vec::into_iter::IntoIter<T>> for alloc_models::vec::Vec<T>}]
    Source: 'src/lib.rs', lines 726:4-732:5 -/
@[reducible]
def vec.Vec.Insts.CoreIterTraitsCollectIntoIteratorTIntoIter (T : Type) :
  core.iter.traits.collect.IntoIterator (vec.Vec T) T (vec.into_iter.IntoIter
  T) := {
  iteratorIteratorInst :=
    vec.into_iter.IntoIter.Insts.CoreIterTraitsIteratorIterator T
  into_iter :=
    vec.Vec.Insts.CoreIterTraitsCollectIntoIteratorTIntoIter.into_iter
}

/-- [alloc_models::vec::from_elem]:
    Source: 'src/lib.rs', lines 734:4-736:5 -/
def vec.from_elem
  {T : Type} (corecloneCloneInst : core.clone.Clone T) (item : T)
  (len : Std.Usize) :
  Result (vec.Vec T)
  := do
  let s ← rust_primitives.sequence.seq_create corecloneCloneInst item len
  ok s

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::new]:
    Source: 'src/lib.rs', lines 740:8-742:9
    Visibility: public -/
def vec.Vec.new (T : Type) : Result (vec.Vec T) := do
  let s ← rust_primitives.sequence.seq_empty T
  ok s

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::with_capacity]:
    Source: 'src/lib.rs', lines 743:8-745:9
    Visibility: public -/
def vec.Vec.with_capacity
  (T : Type) (_c : Std.Usize) : Result (vec.Vec T) := do
  vec.Vec.new T

/-- [alloc_models::vec::{impl core::default::Default for alloc_models::vec::Vec<T>}::default]:
    Source: 'src/lib.rs', lines 750:8-752:9
    Visibility: public -/
def vec.Vec.Insts.CoreDefaultDefault.default
  (T : Type) : Result (vec.Vec T) := do
  vec.Vec.new T

/-- Trait implementation: [alloc_models::vec::{impl core::default::Default for alloc_models::vec::Vec<T>}]
    Source: 'src/lib.rs', lines 749:4-753:5 -/
@[reducible]
def vec.Vec.Insts.CoreDefaultDefault (T : Type) : core.default.Default (vec.Vec
  T) := {
  default := vec.Vec.Insts.CoreDefaultDefault.default T
}

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::push]:
    Source: 'src/lib.rs', lines 761:8-763:9
    Visibility: public -/
def vec.Vec.push
  {T : Type} (self : vec.Vec T) (x : T) : Result (vec.Vec T) := do
  let s ← rust_primitives.sequence.seq_push self x
  ok s

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::pop]:
    Source: 'src/lib.rs', lines 764:8-772:9
    Visibility: public -/
def vec.Vec.pop
  {T : Type} (self : vec.Vec T) :
  Result ((core.option.Option T) × (vec.Vec T))
  := do
  let l ← rust_primitives.sequence.seq_len self
  if l > 0#usize
  then
    let i ← l - 1#usize
    let (last, s) ← rust_primitives.sequence.seq_remove self i
    ok (core.option.Option.Some last, s)
  else ok (core.option.Option.None, self)

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::is_empty]:
    Source: 'src/lib.rs', lines 773:8-775:9
    Visibility: public -/
def vec.Vec.is_empty {T : Type} (self : vec.Vec T) : Result Bool := do
  let i ← rust_primitives.sequence.seq_len self
  ok (i = 0#usize)

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::insert]:
    Source: 'src/lib.rs', lines 777:8-782:9
    Visibility: public -/
def vec.Vec.insert
  {T : Type} (self : vec.Vec T) (index : Std.Usize) (element : T) :
  Result (vec.Vec T)
  := do
  let l ← rust_primitives.sequence.seq_len self
  let (right, s) ← rust_primitives.sequence.seq_drain self index l
  let s1 ← rust_primitives.sequence.seq_push s element
  let (s2, _) ← rust_primitives.sequence.seq_concat s1 right
  ok s2

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::truncate]:
    Source: 'src/lib.rs', lines 789:8-794:9
    Visibility: public -/
def vec.Vec.truncate
  {T : Type} (self : vec.Vec T) (n : Std.Usize) : Result (vec.Vec T) := do
  let l ← rust_primitives.sequence.seq_len self
  if n < l
  then let (_, s) ← rust_primitives.sequence.seq_drain self n l
       ok s
  else ok self

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::swap_remove]:
    Source: 'src/lib.rs', lines 797:8-807:9
    Visibility: public -/
def vec.Vec.swap_remove
  {T : Type} (self : vec.Vec T) (n : Std.Usize) :
  Result (T × (vec.Vec T))
  := do
  let l ← rust_primitives.sequence.seq_len self
  let i ← l - 1#usize
  let (last, s) ← rust_primitives.sequence.seq_remove self i
  if n = i
  then ok (last, s)
  else
    let (removed, s1) ← rust_primitives.sequence.seq_remove s n
    let self1 ← vec.Vec.insert s1 n last
    ok (removed, self1)

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::remove]:
    Source: 'src/lib.rs', lines 814:8-816:9
    Visibility: public -/
def vec.Vec.remove
  {T : Type} (self : vec.Vec T) (index : Std.Usize) :
  Result (T × (vec.Vec T))
  := do
  let (t, s) ← rust_primitives.sequence.seq_remove self index
  ok (t, s)

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::clear]:
    Source: 'src/lib.rs', lines 818:8-820:9
    Visibility: public -/
def vec.Vec.clear {T : Type} (self : vec.Vec T) : Result (vec.Vec T) := do
  let s ← rust_primitives.sequence.seq_empty T
  ok s

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::append]:
    Source: 'src/lib.rs', lines 822:8-825:9
    Visibility: public -/
def vec.Vec.append
  {T : Type} (self : vec.Vec T) (other : vec.Vec T) :
  Result ((vec.Vec T) × (vec.Vec T))
  := do
  let (s, _) ← rust_primitives.sequence.seq_concat self other
  let s1 ← rust_primitives.sequence.seq_empty T
  ok (s, s1)

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::split_off]:
    Source: 'src/lib.rs', lines 829:8-832:9
    Visibility: public -/
def vec.Vec.split_off
  {T : Type} (self : vec.Vec T) (at1 : Std.Usize) :
  Result ((vec.Vec T) × (vec.Vec T))
  := do
  let l ← rust_primitives.sequence.seq_len self
  let (s, s1) ← rust_primitives.sequence.seq_drain self at1 l
  ok (s, s1)

/-- [alloc_models::vec::drain::{impl core::iter::traits::iterator::Iterator<T> for alloc_models::vec::drain::Drain<T, A>}::next]:
    Source: 'src/lib.rs', lines 850:12-857:13
    Visibility: public -/
def vec.drain.Drain.Insts.CoreIterTraitsIteratorIterator.next
  {T : Type} {A : Type} (self : vec.drain.Drain T A) :
  Result ((core.option.Option T) × (vec.drain.Drain T A))
  := do
  let (s, pd) := self
  let i ← rust_primitives.sequence.seq_len s
  if i = 0#usize
  then ok (core.option.Option.None, self)
  else
    let (res, s1) ← rust_primitives.sequence.seq_remove s 0#usize
    ok (core.option.Option.Some res, (s1, pd))

/-- Trait implementation: [alloc_models::vec::drain::{impl core::iter::traits::iterator::Iterator<T> for alloc_models::vec::drain::Drain<T, A>}]
    Source: 'src/lib.rs', lines 848:8-858:9 -/
@[reducible]
def vec.drain.Drain.Insts.CoreIterTraitsIteratorIterator (T : Type) (A : Type)
  : core.iter.traits.iterator.Iterator (vec.drain.Drain T A) T := {
  next := vec.drain.Drain.Insts.CoreIterTraitsIteratorIterator.next
}

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::extend_from_slice]:
    Source: 'src/lib.rs', lines 868:8-870:9 -/
def vec.Vec.extend_from_slice
  {T : Type} (corecloneCloneInst : core.clone.Clone T) (self : vec.Vec T)
  (other : Slice T) :
  Result (vec.Vec T)
  := do
  let s ← rust_primitives.sequence.seq_extend corecloneCloneInst self other
  ok s

/-- [alloc_models::vec::{alloc_models::vec::Vec<T>}::resize]:
    Source: 'src/lib.rs', lines 875:8-883:9
    Visibility: public -/
def vec.Vec.resize
  {T : Type} (corecloneCloneInst : core.clone.Clone T) (self : vec.Vec T)
  (new_size : Std.Usize) (value : T) :
  Result (vec.Vec T)
  := do
  let l ← rust_primitives.sequence.seq_len self
  if new_size > l
  then
    let i ← new_size - l
    let extra ←
      rust_primitives.sequence.seq_create corecloneCloneInst value i
    let (s, _) ← rust_primitives.sequence.seq_concat self extra
    ok s
  else let (_, s) ← rust_primitives.sequence.seq_drain self new_size l
       ok s

/-- Trait implementation: [alloc_models::vec::{impl core::ops::index::Index<I, Clause0_Output> for alloc_models::vec::Vec<T>}]
    Source: 'src/lib.rs', lines 899:4-908:5 -/
@[reducible]
def vec.Vec.Insts.CoreOpsIndexIndex {T : Type} {I : Type} {Clause0_Output :
  Type} (coresliceindexSliceIndexISliceClause0_OutputInst :
  core.slice.index.SliceIndex I (Slice T) Clause0_Output) :
  core.ops.index.Index (vec.Vec T) I Clause0_Output := {
  index := vec.Vec.Insts.CoreOpsIndexIndex.index
    coresliceindexSliceIndexISliceClause0_OutputInst
}

/-- [alloc_models::vec::{impl core::ops::index::IndexMut<I, Clause0_Output> for alloc_models::vec::Vec<T>}::index_mut]:
    Source: 'src/lib.rs', lines 921:8-923:9
    Visibility: public -/
def vec.Vec.Insts.CoreOpsIndexIndexMut.index_mut
  {T : Type} {I : Type} {Clause0_Output : Type}
  (coresliceindexSliceIndexISliceClause0_OutputInst :
  core.slice.index.SliceIndex I (Slice T) Clause0_Output) (self : vec.Vec T)
  (i : I) :
  Result (Clause0_Output × (Clause0_Output → vec.Vec T))
  := do
  let (s, seq_to_slice_mut_back) ←
    rust_primitives.sequence.seq_to_slice_mut self
  let (t, index_mut_back) ←
    core.Slice.Insts.CoreOpsIndexIndexMut.index_mut
      coresliceindexSliceIndexISliceClause0_OutputInst s i
  let back :=
    fun t1 => let s1 := index_mut_back t1
              let s2 := seq_to_slice_mut_back s1
              s2
  ok (t, back)

/-- Trait implementation: [alloc_models::vec::{impl core::ops::index::IndexMut<I, Clause0_Output> for alloc_models::vec::Vec<T>}]
    Source: 'src/lib.rs', lines 915:4-924:5 -/
@[reducible]
def vec.Vec.Insts.CoreOpsIndexIndexMut {T : Type} {I : Type} {Clause0_Output :
  Type} (coresliceindexSliceIndexISliceClause0_OutputInst :
  core.slice.index.SliceIndex I (Slice T) Clause0_Output) :
  core.ops.index.IndexMut (vec.Vec T) I Clause0_Output := {
  IndexInst := vec.Vec.Insts.CoreOpsIndexIndex
    coresliceindexSliceIndexISliceClause0_OutputInst
  index_mut := vec.Vec.Insts.CoreOpsIndexIndexMut.index_mut
    coresliceindexSliceIndexISliceClause0_OutputInst
}

/-- Trait implementation: [alloc_models::vec::{impl core::ops::deref::Deref<[T]> for alloc_models::vec::Vec<T>}]
    Source: 'src/lib.rs', lines 927:4-933:5 -/
@[reducible]
def vec.Vec.Insts.CoreOpsDerefDerefSlice (T : Type) : core.ops.deref.Deref
  (vec.Vec T) (Slice T) := {
  deref := vec.Vec.Insts.CoreOpsDerefDerefSlice.deref
}

end alloc_models
