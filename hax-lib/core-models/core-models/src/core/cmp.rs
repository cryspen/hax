...
mod tests {
    ...
    // The scalar `PartialEq` / `PartialOrd` / `Ord` impls are generated by
    // `int_impls!` for all twelve integer types, so exercise every one of
    // them rather than `u8` alone. Signed types matter in particular: their
    // Lean counterparts are extracted from these bodies, and a signed-vs-
    // bitvector comparison mix-up would only show up here.
    macro_rules! int_cmp_test {
        ($($t:ty)*) => {
            paste! {
                $(
                    proptest! {
                        #[test]
                        fn [<test_ $t _partial_cmp>](x in any::<$t>(), y in any::<$t>()) {
                            prop_assert_eq!(
                                <$t as PartialOrd<$t>>::partial_cmp(&x.inject(), &y.inject()),
                                std::cmp::PartialOrd::partial_cmp(&x, &y).inject()
                            );
                        }

                        #[test]
                        fn [<test_ $t _cmp>](x in any::<$t>(), y in any::<$t>()) {
                            prop_assert_eq!(
                                <$t as Ord>::cmp(&x.inject(), &y.inject()),
                                std::cmp::Ord::cmp(&x, &y).inject()
                            );
                        }

                        #[test]
                        fn [<test_ $t _eq>](x in any::<$t>(), y in any::<$t>()) {
                            prop_assert_eq!(
                                <$t as PartialEq<$t>>::eq(&x.inject(), &y.inject()),
                                std::cmp::PartialEq::eq(&x, &y)
                            );
                        }
                    }
                )*
    ...
    int_cmp_test! { u8 u16 u32 u64 u128 usize i8 i16 i32 i64 i128 isize }
    ...
    proptest! {
        ...
        // `partial_cmp` / `cmp` / `eq` for every integer type: `int_cmp_test!`.
