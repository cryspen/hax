...
let impl_2__extend_from_slice #t (#[(Tactics.exact (`()))]alloc:unit) (self: t_Vec t alloc) (other: t_Slice t{length self + length other <= max_usize}): t_Vec t alloc
  = concat self other
...
let impl_1__push #t
  ...
  (x: t)
   : t_Vec t alloc = 
     if length v = max_usize then v else
     concat v (create 1 x)
...
let from_elem #a (item: a) (len: usize) = create (v len) item
...
instance update_at_tc_array t n: update_at_tc (t_Vec t ()) (int_t n) = {
  ...
  update_at = (fun arr i x -> upd arr (v i) x);
}
...
let impl_1__insert #t (#[(Tactics.exact (`()))]alloc:unit) (vec: t_Vec t alloc) 
  (index: usize {v index <= length vec /\ length vec < max_usize}) (element: t) 
  : t_Vec t alloc =
  let left = slice #t vec 0 (v index) in 
  let right = slice #t vec (v index) (length vec) in 
...
assume val impl_2__resize #t (#[(Tactics.exact (`()))]alloc:unit)  (vec: t_Vec t alloc) (new_size: usize) (value: t): 
  Prims.Pure
      (t_Vec t alloc)
      Prims.l_True
      (ensures
