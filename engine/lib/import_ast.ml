...
and dglobal_ident (gi : A.global_id) : B.global_ident =
  match gi with
  | Types.Concrete (Concrete c) -> `Concrete (Concrete_ident.from_rust_ast c)
  | Types.Concrete (Tuple (Type { length })) ->
      let length = Int.of_string length in
      `TupleType length
  | Types.Concrete (Tuple (Constructor { length })) ->
      let length = Int.of_string length in
      `TupleCons length
  | Types.Concrete (Tuple (Field { length; nth })) ->
      let length = Int.of_string length in
      let nth = Int.of_string nth in
      `TupleField (nth, length)
  | Types.Projector (Concrete c) ->
      `Projector (`Concrete (Concrete_ident.from_rust_ast c))
  | Types.Projector (Tuple (Field { length; nth })) ->
      let length = Int.of_string length in
      let nth = Int.of_string nth in
      `Projector (`TupleField (nth, length))
  | Types.Projector (Tuple (Constructor _ | Type _)) ->
      broken_invariant "Projectors from tuple constructor or type"
