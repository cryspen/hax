...
module Make (FA : Features.T) (FB : Features.T) = struct
  ...
  let rec dty (ty : A.ty) : B.ty =
    ...
    | TRef { witness = _; typ; mut; region } -> Ref {
      inner = dty typ; 
      mutable' = match mut with | Mutable _ -> true | _ -> false}
    | TParam local_ident -> Param (dlocal_ident local_ident)
    | TArrow (inputs, output) -> Arrow {inputs = List.map ~f:dty inputs; output = dty output}
  ...
  and dint_kind (ik: int_kind): B.int_kind =
    let size: B.int_size = match ik.size with 
      | S8 -> S8
      | S16 -> S16
      | S32 -> S32
      | S64 -> S64
      ...
      | SSize -> SSize in
    {
      size
      ;
      signedness = match ik.signedness with 
      | Signed -> Signed
      | Unsigned -> Unsigned
    }
  ...
  and dimpl_expr (i : A.impl_expr) : B.impl_expr = 
    { goal = dtrait_goal i.goal;
    kind = dimpl_expr_kind i.kind
    }
  ...
  | A.Self -> B.Self_
  | A.Concrete tr ->
      B.Concrete (dtrait_goal tr)
  | A.LocalBound { id } ->
      B.LocalBound { id = B.Newtypesymbol id }
  | A.Parent { impl; ident } ->
      B.Parent {
        impl_ = dimpl_expr impl;
        item = todo (); (* Rust AST needs fixing*)
      }
  ...
  | A.ImplApp { impl; args } ->
      B.ImplApp {
        impl_ = dimpl_expr impl;
        args = List.map ~f:dimpl_expr args;
      }
  | A.Dyn -> B.Dyn
  | A.Builtin tr ->
      B.Builtin (dtrait_goal tr)
  ...
  and dmetadata ?(attrs = []) (span: span) : B.metadata =
    {
      attributes = List.map ~f:(dattr) attrs;
      span = dspan span
    }
    and dattr (a: attr) : B.attribute =
      let kind = match a.kind with 
        | Tool {path; tokens} -> B.Tool {path; tokens}
        | DocComment {kind; body} -> 
          let kind =  
        match kind with |DCKLine -> B.Line | DCKBlock -> Block in
          B.DocComment {kind; body} in
      {
        kind ;
        span = dspan a.span
      }
  ...
  | PConstruct { constructor; is_record; is_struct; fields } ->
      Construct {
        ...
        is_record;
        is_struct;
        fields = List.map ~f:(fun { field; pat } -> (dglobal_ident field, dpat pat)) fields;
      }
  | POr { subpats } ->
      Or { sub_pats = List.map ~f:dpat subpats }
  | PArray { args } ->
      Array { args = List.map ~f:dpat args }
  | PDeref { subpat; witness = _ } ->
      Deref { sub_pat = dpat subpat }
  | PConstant { lit } ->
      Constant { lit = dliteral lit }
  ...
  and dexpr (e : A.expr) : B.expr = {
    kind = dexpr' e.e;
    ty = dty e.typ;
    meta = dmetadata e.span
  }

  and dexpr' (expr : A.expr') : B.expr_kind =
      match expr with
  | If { cond; then_; else_ } ->
      If { condition = dexpr cond; then' = dexpr then_; else_ = Option.map ~f:dexpr else_ }
  | App { f; args; generic_args; bounds_impls; trait } ->
      App {
        ...
        trait_ = Option.map ~f:(fun (impl, args) -> (dimpl_expr impl, List.map ~f:dgeneric_value args)) trait;
      }
  ...
  | Construct { constructor; is_record; is_struct; fields; base } ->
      Construct {
        constructor = dglobal_ident constructor;
        fields = List.map ~f:(fun (id, e) -> (dglobal_ident id, dexpr e)) fields;
  ...
  | Block { e; safety_mode = _; witness = _ } -> todo () (* Rust AST needs fixing *)
  ...
  | Loop { body; kind; state; control_flow; label; witness = _ } ->
      Loop {
        ...
        control_flow = Option.map ~f:(fun (k, _) -> dcontrol_flow_kind k) control_flow;
        label = Option.map ~f:(fun s -> B.Newtypesymbol s) label;
      }
  | Break { e; acc = _; label; witness = _ } ->
      Break { value = dexpr e; label = Option.map ~f:(fun s -> B.Newtypesymbol s) label }
  ...
  | Borrow { kind; e; witness = _ } ->
      Borrow { inner = dexpr e; mutable' = match kind with Mut _ -> true | _ -> false }
  | AddressOf { mut; e; witness = _ } ->
      AddressOf { inner = dexpr e; mutability = match mut with Mutable _ -> true | _ -> false  }
| Closure { params; body; captures } ->
  ...
  | EffectAction { action = _; argument } ->
      todo ()
  | Quote q -> Quote { contents = dquote q }

  and dcontrol_flow_kind (cfk: A.cf_kind): B.control_flow_kind = match cfk with
  | BreakOnly -> B.BreakOnly
  | BreakOrReturn -> B.BreakOrReturn

  and dliteral (l: Ast.literal): B.literal = match l with 
  | String s -> B.String (Newtypesymbol s)
    | Char c -> B.Char c
      | Int {value; negative; kind} -> B.Int {value; negative; kind = dint_kind kind}
        | Float {value; negative; kind} -> B.Float {value = Newtypesymbol value; negative; kind = dfloat_kind kind}
        | Bool b -> B.Bool b
  ...
  and ditem_quote_origin (iqo: item_quote_origin): B.item_quote_origin =
    {
      ...
      item_kind = todo (); (*Rust AST needs fixing*)
      position = match iqo.position with 
      | `Before -> B.Before | `After -> B.After | `Replace -> B.Replace
    }
  ...
  and dguard' (guard : A.guard') : B.guard_kind =
    match guard with IfLet { lhs; rhs; witness = _ } -> B.IfLet {lhs = dpat lhs; rhs = dexpr rhs}

  and dlhs (lhs : A.lhs) : B.lhs =
    match lhs with
  | A.LhsLocalVar { var; typ } ->
      B.LocalVar { var = dlocal_ident var; ty = dty typ }
  | A.LhsArbitraryExpr { e; witness = _ } ->
      B.ArbitraryExpr (dexpr e)
  ...
  | A.Impl {
      ...
      B.Impl {
        ...
        self_ty = dty self_ty;
        of_trait =
          [ (dconcrete_ident trait_id, todo ()(* List.map ~f:dgeneric_value trait_generics *)) ]; (* Rust AST needs fixing*)
        items = List.map ~f:dimpl_item items;
        parent_bounds = List.map ~f:(fun (impl, ident) ->
          (dimpl_expr impl, dimpl_ident ident)) parent_bounds;
        safety = dsafety_kind safety;
      }
  ...
  | A.Use { path; is_external; rename } ->
      B.Use { path; is_external; rename }

  | A.Quote { quote; origin } ->
      B.Quote {
        quote = dquote quote;
        origin = ditem_quote_origin origin;
      }
