...
## Procedure
 ...
 3. Update the aeneas/charon pins:
    - Set `aeneas-pin` to the latest release tag from [AeneasVerif/aeneas](https://github.com/AeneasVerif/aeneas/releases).
    - Set `charon-pin` to the release tag matching the charon commit pinned by that aeneas version (check `charon-pin` in the aeneas repo at the pinned commit). Update the `version` line to match `charon version` output for that release.
