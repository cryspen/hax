    ...
    fn visit_item_mut(&mut self, item: &mut syn::Item) {
        ...
        *item = parse_quote! {
            impl<'a> ToPrintView<'a> for origin::#type_ident {
                type Viewed = destination::#type_ident #generics_destination;
                fn to_print_view(&'a self, #[allow(unused_variables)] parent_context: Option<std::rc::Rc<ParentPrintContext<'a>>>) -> Self::Viewed {
                    match self {
                        #(#branches),*
                    }
                }
